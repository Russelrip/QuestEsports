...
rehearsal_canonical_acl_rows() {
  ...
  #
  # quest_backup appears here because the bootstrap now provisions it. It used
  # to be absent: production granted the backup reader default privileges the
  # canonical bootstrap never re-created, so a recovered database silently lost
  # them and tables created after a recovery fell out of the backup. The
  # bootstrap grants it after the runtime role, which is why quest_backup is
  # the second aclitem in each array.
  printf '%s\n' \
