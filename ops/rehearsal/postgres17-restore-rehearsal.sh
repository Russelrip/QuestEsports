...
runtime_psql_query() { env -u PGUSER -u PGDATABASE -u PGHOST -u PGPORT -u PGSERVICE -u PGOPTIONS -u PGPASSWORD PGAPPNAME="${rehearsal_app_name:-quest-rehearsal-read-probe}" psql -X -A -t -F '|' -v ON_ERROR_STOP=1 "$runtime_db_url" -c "$2" >"$1" 2>/dev/null; }
...
psql_query "$scratch/target-identity" "SELECT current_database() || '|' || current_setting('server_version_num') || '|' || current_setting('ssl')" || fail "target database identity query failed"
IFS='|' read -r target_identity_database target_identity_server_version target_identity_ssl < "$scratch/target-identity"
[[ "$target_identity_database" == "$target_database" && "$target_identity_server_version" =~ ^17[0-9]{4}$ && "$target_identity_ssl" == on ]] || fail "target database identity or PostgreSQL security setting is unsafe"
...
[[ "$(tr -d '\r' < "$scratch/quest-write-probe")" == 'quest_runtime|quest_runtime|write_verified' ]] || fail "Quest runtime write probe returned an invalid authenticated role or status"
...
while IFS=$'\t' read -r schema table protected policy roles command qual with_check; do [[ "$schema" == public || "$schema" == valorant ]] || fail "RLS schema is unsafe"; [[ -n "$table" && "$protected" == t || "$protected" == f ]] || fail "RLS table row is malformed"; expected_runtime_role="$(rehearsal_runtime_role "$schema")" || fail "RLS schema has no runtime role"; expected_runtime_policy="$(rehearsal_runtime_policy_name "$schema" "$table")" || fail "RLS table has no canonical runtime policy name"; if [[ "$schema" == valorant && "$table" != _migration_ledger ]]; then [[ "$protected" == t && "$policy" == "$expected_runtime_policy" && "$roles" == *"$expected_runtime_role"* && "$command" == 'ALL' ]] || fail "VALORANT table lacks its val_runtime RLS policy"; elif [[ "$schema" == public && "$table" != _prisma_migrations ]]; then [[ "$protected" == t && "$policy" == "$expected_runtime_policy" && "$roles" == *"$expected_runtime_role"* && "$command" == 'ALL' ]] || fail "Quest table lacks its quest_runtime RLS policy"; fi; done < "$scratch/rls"
...
(( public_policy_count > 0 && valorant_policy_count > 0 )) || fail "runtime policy coverage is incomplete"
