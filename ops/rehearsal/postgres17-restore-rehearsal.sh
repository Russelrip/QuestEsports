...
psql_query "$scratch/ext" "SELECT extname || '|' || extversion FROM pg_extension ORDER BY extname" || fail "extension inventory query failed"; extensions_sha256="$(sha256sum "$scratch/ext" | cut -d' ' -f1)"; grep -Eq '^[a-zA-Z0-9_]+\|[^[:space:]]+$' "$scratch/ext" || fail "extension inventory was malformed"; extensions_count="$(wc -l < "$scratch/ext" | tr -d ' ')"; [[ "$extensions_count" =~ ^[0-9]+$ ]] || fail "extension count was malformed"
