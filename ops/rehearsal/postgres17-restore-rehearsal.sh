...
runtime_psql_query() { local output="$1" sql="$2" app="quest-runtime-probe-${rehearsal_nonce_after:-$rehearsal_nonce}"; runtime_psql_last_app="$app"; env -u PGUSER -u PGDATABASE -u PGHOST -u PGPORT -u PGSERVICE -u PGOPTIONS -u PGPASSWORD PGAPPNAME="$app" psql -X -q -A -t -F '|' -v ON_ERROR_STOP=1 -v VERBOSITY=sqlstate "$runtime_db_url" -c "$sql" >"$output" 2>"$output.stderr"; }
...
runtime_psql_query "$scratch/quest-write-probe" "BEGIN; INSERT INTO public.\"users\" (\"id\",\"first_name\",\"last_name\",\"email\",\"email_normalized\",\"username\",\"username_normalized\",\"password_hash\",\"role\",\"updated_at\") VALUES ('00000000-0000-0000-0000-000000000017','Quest','Rehearsal','quest-rehearsal-probe@example.invalid','quest-rehearsal-probe@example.invalid','quest_rehearsal_probe','quest_rehearsal_probe','rehearsal-probe','user',CURRENT_TIMESTAMP); SELECT session_user || '|' || current_user || '|write_verified'; ROLLBACK" || fail "Quest runtime database-backed write probe failed"
write_probe_row='quest_runtime|quest_runtime|write_verified'
[[ "$(grep -Fxc "$write_probe_row" "$scratch/quest-write-probe" || true)" == 1 ]] || fail "Quest runtime write probe returned an invalid authenticated role or status"
...
if runtime_psql_query "$scratch/cross-schema-denial" "SELECT count(*) FROM valorant.\"_migration_ledger\""; then fail "quest_runtime can reach the VALORANT schema"; fi
denial_stderr="$scratch/cross-schema-denial.stderr"
[[ -s "$denial_stderr" ]] || fail "cross-schema denial produced no diagnostic"
grep -Eq '^ERROR:[[:space:]]+42501[[:space:]]*$' "$denial_stderr" || fail "cross-schema denial did not return SQLSTATE 42501"
