...
[[ "$(grep -Exc "$write_probe_row_regex" "$scratch/quest-write-probe" || true)" == 1 ]] || fail "Quest runtime write probe returned an invalid authenticated role, nonce session, or TLS status"
