...
for command in realpath sha256sum age tar mktemp date grep sed find sort stat curl python3 rsync sleep; do need "$command"; done
...
source_major="${SOURCE_POSTGRES_MAJOR:-${BACKUP_SOURCE_MAJOR:-}}"; source_version="${SOURCE_POSTGRES_VERSION:-${BACKUP_SOURCE_VERSION:-PostgreSQL $source_major}}"
[[ "$source_major" =~ ^[0-9]+$ && "$source_major" -gt 0 && "$source_version" != *$'\n'* ]] || fail "source PostgreSQL major record is missing or unsafe"
...
ledger() { local schema="$1" out="$2" candidate count; for candidate in _prisma_migrations alembic_version schema_migrations; do if psql_query "$out" "SELECT count(*) FROM \"$schema\".\"$candidate\""; then count="$(tr -d '[:space:]' < "$out")"; [[ "$count" =~ ^[0-9]+$ && "$count" -gt 0 ]] && { printf '%s' "$count"; return; }; fi; done; return 1; }
quest_migrations="$(ledger public "$scratch/q-ledger")" || fail "Quest migration ledger was not verified"; valorant_migrations="$(ledger valorant "$scratch/v-ledger")" || fail "VALORANT migration ledger was not verified"
...
psql_query "$scratch/ext" "SELECT count(*) FROM pg_extension" || fail "extension query failed"; extensions="$(tr -d '[:space:]' < "$scratch/ext")"; [[ "$extensions" =~ ^[0-9]+$ ]] || fail "extension result malformed"
psql_query "$scratch/settings" "SELECT current_setting('server_version_num')" || fail "settings query failed"; settings="$(tr -d '[:space:]' < "$scratch/settings")"; [[ "$settings" =~ ^17[0-9]{3}$ ]] || fail "target PostgreSQL server is not major 17"; target_major=17
...
[[ "${NO_WRITER_ADMISSION_CONFIRMATION:-}" == verified ]] || fail "no-writer-admission confirmation is required"
for injection in bad_checksum bad_decryption wrong_ca blocked_network failed_service_health attempted_mutation_callback; do var="FAILURE_INJECTION_${injection^^}_STATUS"; status="${!var:-}"; [[ "$status" == passed ]] || fail "failure injection $injection is not passed"; done
