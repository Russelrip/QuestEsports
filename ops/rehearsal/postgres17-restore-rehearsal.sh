...
parent_device="$(stat -c '%d' -- "$upload_parent")"; parent_uid="$(stat -c '%u' -- "$upload_parent")"
[[ "$parent_device" =~ ^[0-9]+$ && "$parent_uid" == "$(id -u)" ]] || fail "disposable upload parent ownership/device is unsafe"
for root in "$public_resolved" "$private_resolved"; do [[ -d "$root" && ! -L "$root" && "$(stat -c '%d' -- "$root")" == "$parent_device" && "$(stat -c '%u' -- "$root")" == "$(id -u)" && "$(stat -c '%a' -- "$root")" == 700 && -z "$(find -P "$root" -mindepth 1 -print -quit)" ]] || fail "disposable upload root ownership/device/freshness is unsafe"; done
...
[[ "$inspected_id" == "${sentinel_cfg[container_id]}" && "$inspected_name" == /quest-rehearsal-* && "$inspected_running" == true && "$inspected_image" =~ ^postgres:17([.@].*)?$ && "$container_disk_bytes" =~ ^[0-9]+$ ]] || fail "disposable target container identity and PostgreSQL 17 image are unsafe"
...
validate_extensions() { local file="$1" count; grep -Eq '^[A-Za-z0-9_]+\|[^[:space:]]+$' "$file" || return 1; grep -qx 'plpgsql|1.0' "$file" || return 1; count="$(wc -l < "$file" | tr -d ' ')"; [[ "$count" =~ ^[0-9]+$ && "$count" -ge 1 ]]; }
...
[[ "$inspected_id_after" == "${sentinel_cfg[container_id]}" && "$inspected_name_after" == /quest-rehearsal-* && "$inspected_running_after" == true && "$inspected_image_after" =~ ^postgres:17([.@].*)?$ && "$container_disk_bytes_after" =~ ^[0-9]+$ ]] || fail "post-restore target container identity and PostgreSQL 17 image are unsafe"
...
psql_query "$scratch/quest-read-probe" "SELECT count(*) FROM public.\"users\"" || fail "Quest database-backed read probe failed"
...
validate_failure_output() {
  local injection="$1" output="$2" expected_class
  ...
  [[ "${failure[result]}" == passed && "${failure[failure_class]}" == "$expected_class" && "${failure[service_identity]}" == "$expected_service" && "${failure[endpoint_id]}" =~ ^[A-Za-z0-9_.-]+$ && "${failure[endpoint_sha256]}" =~ ^[a-fA-F0-9]{64}$ && "${failure[pre_state]}" =~ ^(running|stopped|blocked|unavailable)$ && "${failure[post_state]}" =~ ^(running|stopped|blocked|unavailable)$ && "${failure[containment]}" == verified ]]
}
...
for injection in bad_checksum bad_decryption wrong_ca blocked_network failed_service_health attempted_mutation_callback; do
  ...
  validate_failure_output "$injection" "$scratch/$injection.output" || fail "failure injection $injection evidence is not structured or contained"
done
failure_inventory="$scratch/failure-injections.tsv"
: > "$failure_inventory"
for injection in bad_checksum bad_decryption wrong_ca blocked_network failed_service_health attempted_mutation_callback; do
  var="FAILURE_INJECTION_${injection^^}_COMMAND"; hook="${!var}"
  printf '%s|%s|%s|passed|%s\n' "$injection" "$hook" "$(sha256sum "$hook" | cut -d' ' -f1)" "$(sha256sum "$scratch/$injection.output" | cut -d' ' -f1)" >> "$failure_inventory"
done
