...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  case "$key" in
    RECOVERY_ADMIN_URL|RECOVERY_CLIENT_CERT_FILE|RECOVERY_CLIENT_KEY_FILE|QUEST_RUNTIME_DATABASE_URL|UPLOAD_ROOT|PRIVATE_UPLOAD_ROOT|BACKUP_AGE_IDENTITY_FILE) [[ -z "${cfg[$key]+x}" ]] || fail "BACKUP_ENV_FILE contains a duplicate"; cfg["$key"]="$value" ;;
...
for key in RECOVERY_ADMIN_URL RECOVERY_CLIENT_CERT_FILE RECOVERY_CLIENT_KEY_FILE UPLOAD_ROOT PRIVATE_UPLOAD_ROOT BACKUP_AGE_IDENTITY_FILE; do [[ -n "${cfg[$key]:-}" ]] || fail "BACKUP_ENV_FILE is missing a required setting"; done
db_url="${cfg[RECOVERY_ADMIN_URL]}"; recovery_client_cert_file="${cfg[RECOVERY_CLIENT_CERT_FILE]}"; recovery_client_key_file="${cfg[RECOVERY_CLIENT_KEY_FILE]}"; runtime_db_url="${cfg[QUEST_RUNTIME_DATABASE_URL]:-}"; public_root="${cfg[UPLOAD_ROOT]}"; private_root="${cfg[PRIVATE_UPLOAD_ROOT]}"; identity="${cfg[BACKUP_AGE_IDENTITY_FILE]}"
...
check_path "$recovery_client_cert_file" RECOVERY_CLIENT_CERT_FILE; check_path "$recovery_client_key_file" RECOVERY_CLIENT_KEY_FILE
[[ -f "$recovery_client_cert_file" && -f "$recovery_client_key_file" && -r "$recovery_client_cert_file" && -r "$recovery_client_key_file" ]] || fail "recovery client TLS material is missing or unreadable"
private "$recovery_client_cert_file" || fail "recovery client certificate is not private"
private "$recovery_client_key_file" || fail "recovery client key is not private"
...
upload_cleanup_status=pending
cleanup_upload_parent() {
  (( ${upload_parent_created:-0} )) || return 0
  [[ -d "$upload_parent" && ! -L "$upload_parent" ]] || return 1
  for root in "$public_resolved" "$private_resolved"; do
    [[ -d "$root" && ! -L "$root" ]] || return 1
    [[ "$(stat -c '%d:%u:%g:%a' -- "$root")" == "$parent_device:$parent_uid:$parent_gid:700" ]] || return 1
    [[ -z "$(find -P "$root" -type l -print -quit)" ]] || return 1
  done
  while IFS= read -r -d '' child; do
    child_name="$(basename "$child")"
    case "$child_name" in
      "$(basename "$public_resolved")"|"$(basename "$private_resolved")"|.quest-previous-"$(basename "$public_resolved")"-*|.quest-previous-"$(basename "$private_resolved")"-*|.quest-restore-"$(basename "$public_resolved")"-*|.quest-restore-"$(basename "$private_resolved")"-*) rm -rf -- "$child" || return 1 ;;
      *) return 1 ;;
    esac
  done < <(find -P "$upload_parent" -mindepth 1 -maxdepth 1 -print0)
  [[ ! -e "$public_resolved" && ! -e "$private_resolved" ]] || return 1
  rmdir -- "$upload_parent" || return 1
  [[ ! -e "$upload_parent" && ! -L "$upload_parent" ]] || return 1
  upload_cleanup_status=verified; upload_parent_created=0
}
cleanup() { local s=$?; set +e; stop_binding_session; rm -f -- "$evidence"/.rehearsal-failure-hook-*.sh; rm -rf -- "$scratch"; if ! cleanup_upload_parent; then echo 'Restore rehearsal cleanup failed: disposable upload roots were not removed' >&2; fi; return "$s"; }; trap cleanup EXIT
...
start="$(date -u +%s)"; start_utc="$(date -u -d "@$start" +%Y-%m-%dT%H:%M:%SZ)"
...
psql_query() { env -u PGUSER -u PGDATABASE -u PGHOST -u PGPORT -u PGSERVICE -u PGOPTIONS -u PGPASSWORD PGAPPNAME="${rehearsal_app_name:-}" psql -X -A -t -F '|' "$db_url" -c "$2" >"$1" 2>"$1.stderr"; }
runtime_psql_query() { local probe="$1" output="$2" sql="$3" app; if [[ "$probe" == read ]]; then app="quest-runtime-probe-${rehearsal_nonce_after:-$rehearsal_nonce}"; else app="quest-${probe}-probe-${rehearsal_nonce_after:-$rehearsal_nonce}"; fi; runtime_psql_last_app="$app"; env -u PGUSER -u PGDATABASE -u PGHOST -u PGPORT -u PGSERVICE -u PGOPTIONS -u PGPASSWORD PGAPPNAME="$app" psql -X -A -t -F '|' -v ON_ERROR_STOP=1 -v VERBOSITY=verbose "$runtime_db_url" -c "$sql" >"$output" 2>"$output.stderr"; }
...
SELECT current_database() || '|' || current_setting('server_version_num') || '|' || COALESCE((SELECT CASE WHEN ssl THEN 'on' ELSE 'off' END FROM pg_stat_ssl WHERE pid=pg_backend_pid()),'missing') || '|' || current_setting('application_name') || '|' || pg_backend_pid();
SELECT pg_sleep(60);
SQL
  binding_pid=$!
...
validate_binding_output() {
  ...
  [[ "$app" == "$expected_app" && "$database" == "$target_database" && "$session_user" =~ ^[A-Za-z_][A-Za-z0-9_]*$ && "$current_user" == "$session_user" && "$server_port" == 5432 && "$backend_pid" =~ ^[0-9]+$ ]] || return 1
...
validate_container_binding() {
  local output="$1" observed_app observed_database observed_user observed_state observed_port observed_pid
  IFS='|' read -r observed_app observed_database observed_user observed_state observed_port observed_pid < "$output"
  [[ "$observed_app" == "$rehearsal_app_name" && "$observed_database" == "$binding_database" && "$observed_user" == "$binding_session_user" && "$observed_state" == active && "$observed_port" == "$binding_server_port" && "$observed_pid" == "$binding_backend_pid" ]] || return 1
}
...
sed -n '2p' "$scratch/connection-binding" > "$scratch/target-identity"
[[ -s "$scratch/target-identity" ]] || fail "target database identity session probe produced no row"
IFS='|' read -r target_identity_database target_identity_server_version target_identity_ssl target_identity_app target_identity_pid < "$scratch/target-identity"
[[ "$target_identity_database" == "$target_database" && "$target_identity_server_version" =~ ^17[0-9]{4}$ && "$target_identity_ssl" == on && "$target_identity_app" == "$rehearsal_app_name" && "$target_identity_pid" =~ ^[0-9]+$ ]] || fail "target database identity or PostgreSQL session TLS is unsafe"
printf '%s|%s|%s\n' "$target_identity_app" "$target_identity_pid" "$target_identity_ssl" > "$scratch/target-session-tls"
stop_binding_session
...
validate_settings() { local file="$1" count; count="$(wc -l < "$file" | tr -d ' ')"; [[ "$count" == 7 ]] && grep -Eq '^server_version\|17[.][0-9]+(_[A-Za-z0-9_.:+*/-]+)?$' "$file" && grep -Eq '^server_version_num\|17[0-9]{4}$' "$file" && grep -qx 'ssl|on' "$file" && grep -Eq '^ssl_min_protocol_version\|TLSv1\.(2|3)$' "$file" && grep -qx 'row_security|on' "$file" && grep -qx 'default_transaction_read_only|off' "$file" && grep -Eq '^listen_addresses\|([*]|localhost|127\.0\.0\.1)$' "$file"; }
...
roles_nobypassrls_status=verified
...
psql_query "$scratch/object-owners" "WITH relation_owners AS (SELECT n.nspname AS schema_name, c.relname AS object_name, r.rolname AS owner_name, CASE WHEN n.nspname='public' THEN 'quest_migrator' ELSE 'val_migrator' END AS expected_owner FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace JOIN pg_roles r ON r.oid=c.relowner WHERE n.nspname IN ('public','valorant') AND c.relkind IN ('r','p','v','m','S','f','c')), routine_owners AS (SELECT n.nspname AS schema_name, p.proname AS object_name, r.rolname AS owner_name, CASE WHEN n.nspname='public' THEN 'quest_migrator' ELSE 'val_migrator' END AS expected_owner FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace JOIN pg_roles r ON r.oid=p.proowner WHERE n.nspname IN ('public','valorant') AND p.prokind IN ('f','p','a','w')), type_owners AS (SELECT n.nspname AS schema_name, t.typname AS object_name, r.rolname AS owner_name, CASE WHEN n.nspname='public' THEN 'quest_migrator' ELSE 'val_migrator' END AS expected_owner FROM pg_type t JOIN pg_namespace n ON n.oid=t.typnamespace JOIN pg_roles r ON r.oid=t.typowner WHERE n.nspname IN ('public','valorant') AND t.typisdefined AND t.typtype IN ('b','c','d','e','r') AND t.typtype <> 'm' AND t.typelem=0 AND (t.typrelid=0 OR EXISTS (SELECT 1 FROM pg_class composite_relation WHERE composite_relation.oid=t.typrelid AND composite_relation.relkind='c'))) SELECT schema_name || '|' || object_name || '|' || owner_name || '|' || expected_owner FROM (SELECT * FROM relation_owners UNION ALL SELECT * FROM routine_owners UNION ALL SELECT * FROM type_owners) objects WHERE owner_name <> expected_owner ORDER BY schema_name, object_name" || fail "object owner query failed"; [[ ! -s "$scratch/object-owners" ]] || fail "restored object owners do not match their schema migrators"; cat "$scratch/object-owners" >> "$scratch/owners"
# The migration ledgers are deliberately outside the runtime roles' reach --
# they are migrator bookkeeping, not application data -- so asserting every table
# in each schema demanded a grant production does not have and should not have.
# A faithful restore was refused for matching production exactly.
psql_query "$scratch/grants-status" "SELECT CASE WHEN has_schema_privilege('quest_migrator','public','USAGE') AND has_schema_privilege('quest_migrator','public','CREATE') AND has_schema_privilege('quest_runtime','public','USAGE') AND has_schema_privilege('val_migrator','valorant','USAGE') AND has_schema_privilege('val_migrator','valorant','CREATE') AND has_schema_privilege('val_runtime','valorant','USAGE') AND NOT has_schema_privilege('quest_runtime','valorant','USAGE') AND NOT has_schema_privilege('val_runtime','public','USAGE') AND COALESCE((SELECT bool_and(has_table_privilege('quest_runtime', format('%I.%I', n.nspname,c.relname), 'SELECT') AND has_table_privilege('quest_runtime', format('%I.%I', n.nspname,c.relname), 'INSERT') AND has_table_privilege('quest_runtime', format('%I.%I', n.nspname,c.relname), 'UPDATE') AND has_table_privilege('quest_runtime', format('%I.%I', n.nspname,c.relname), 'DELETE')) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind IN ('r','p','f') AND c.relname NOT IN ('_prisma_migrations','_migration_ledger')), false) AND COALESCE((SELECT bool_and(has_table_privilege('val_runtime', format('%I.%I', n.nspname,c.relname), 'SELECT') AND has_table_privilege('val_runtime', format('%I.%I', n.nspname,c.relname), 'INSERT') AND has_table_privilege('val_runtime', format('%I.%I', n.nspname,c.relname), 'UPDATE') AND has_table_privilege('val_runtime', format('%I.%I', n.nspname,c.relname), 'DELETE')) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='valorant' AND c.relkind IN ('r','p','f') AND c.relname NOT IN ('_prisma_migrations','_migration_ledger')), false) AND COALESCE((SELECT bool_and(has_sequence_privilege('quest_runtime', format('%I.%I', n.nspname,c.relname), 'USAGE') AND has_sequence_privilege('quest_runtime', format('%I.%I', n.nspname,c.relname), 'SELECT')) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind='S'), true) AND COALESCE((SELECT bool_and(has_sequence_privilege('val_runtime', format('%I.%I', n.nspname,c.relname), 'USAGE') AND has_sequence_privilege('val_runtime', format('%I.%I', n.nspname,c.relname), 'SELECT')) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='valorant' AND c.relkind='S'), true) THEN 'verified' ELSE 'failed' END" || fail "grant query failed"; [[ "$(tr -d '[:space:]' < "$scratch/grants-status")" == verified ]] || fail "runtime grants or cross-schema denials were not verified"
psql_query "$scratch/extended-object-owners" "SELECT count(*) FROM pg_operator o JOIN pg_namespace n ON n.oid=o.oprnamespace WHERE n.nspname IN ('public','valorant') AND pg_get_userbyid(o.oprowner) <> CASE WHEN n.nspname='public' THEN 'quest_migrator' ELSE 'val_migrator' END" || fail "extended object owner query failed"; [[ "$(tr -d '[:space:]' < "$scratch/extended-object-owners")" == 0 ]] || fail "extended restored object owners do not match their schema migrators"
...
# pg_default_acl stores 0 for a global default, never NULL, so the IS NULL test
# matched nothing and the four global rows were never collected -- leaving the
# inventory permanently four short of the twelve the next line requires.
psql_query "$scratch/acl-global" "SELECT r.rolname || '|<global>|' || d.defaclobjtype::text || '|' || COALESCE(array_to_string(d.defaclacl,','),'') FROM pg_default_acl d JOIN pg_roles r ON r.oid=d.defaclrole WHERE r.rolname IN ('quest_migrator','val_migrator') AND d.defaclnamespace = 0 ORDER BY r.rolname,d.defaclobjtype" || fail "global default ACL inventory query failed"
...
runtime_psql_query read "$scratch/quest-read-probe" "SELECT session_user || '|' || current_user || '|' || count(*) || '|' || current_setting('application_name') || '|' || pg_backend_pid() || '|' || COALESCE((SELECT CASE WHEN ssl THEN 'on' ELSE 'off' END FROM pg_stat_ssl WHERE pid=pg_backend_pid()),'missing') FROM public.\"users\"" || fail "Quest runtime database-backed read probe failed"
...
quest_read_write_probe_status=verified
quest_write_probe_status=verified
if runtime_psql_query denial "$scratch/cross-schema-denial" "SELECT session_user || '|' || current_user || '|cross_schema_denied|' || current_setting('application_name') || '|' || pg_backend_pid() || '|' || COALESCE((SELECT CASE WHEN ssl THEN 'on' ELSE 'off' END FROM pg_stat_ssl WHERE pid=pg_backend_pid()),'missing'); SELECT count(*) FROM valorant.\"_migration_ledger\""; then fail "quest_runtime can reach the VALORANT schema"; fi
denial_stderr="$scratch/cross-schema-denial.stderr"
[[ -s "$denial_stderr" ]] || fail "cross-schema denial produced no diagnostic"
grep -Eq '(^|[^0-9])42501([^0-9]|$)' "$denial_stderr" || fail "cross-schema denial did not return SQLSTATE 42501"
grep -Eiq 'permission denied for (schema valorant|table _migration_ledger)' "$denial_stderr" || fail "cross-schema denial was not the expected permission/schema denial"
! grep -Eiq 'FATAL|authentication|malformed|syntax error|timeout|timed out|could not connect|connection refused' "$denial_stderr" || fail "cross-schema denial included an unexpected failure class"
! grep -Eq '://|(^|[[:space:]])(PASSWORD|TOKEN|SECRET)=' "$denial_stderr" || fail "cross-schema denial diagnostic contains sensitive text"
IFS='|' read -r cross_schema_denial_session_user cross_schema_denial_current_user _ cross_schema_denial_application_name cross_schema_denial_backend_pid cross_schema_denial_ssl < "$scratch/cross-schema-denial"
[[ "$cross_schema_denial_session_user" == quest_runtime && "$cross_schema_denial_current_user" == quest_runtime && "$cross_schema_denial_application_name" == "quest-denial-probe-${rehearsal_nonce_after}" && "$cross_schema_denial_backend_pid" =~ ^[0-9]+$ && "$cross_schema_denial_ssl" == on ]] || fail "cross-schema denial session identity or TLS proof is invalid"
{ printf 'session_user=%s\ncurrent_user=%s\napplication_name=%s\nbackend_pid=%s\nssl=%s\nsqlstate=42501\nerror_class=permission_denied\n' "$cross_schema_denial_session_user" "$cross_schema_denial_current_user" "$cross_schema_denial_application_name" "$cross_schema_denial_backend_pid" "$cross_schema_denial_ssl"; printf 'diagnostic='; tr '\r\n' '  ' < "$denial_stderr" | sed 's/[[:space:]][[:space:]]*/ /g'; printf '\n'; } > "$scratch/cross-schema-denial.tsv"
cross_schema_denial_status=verified
...
public_rls_tables=0; valorant_rls_tables=0; public_policy_rows=0; valorant_policy_rows=0
while IFS=$'\t' read -r schema table protected policy roles command qual with_check; do
  [[ "$schema" == public || "$schema" == valorant ]] || fail "RLS schema is unsafe"
  [[ -n "$table" && ( "$protected" == t || "$protected" == f ) && -n "$policy" && -n "$roles" && -n "$command" && -n "$qual" && -n "$with_check" ]] || fail "RLS table row is malformed"
  expected_runtime_role="$(rehearsal_runtime_role "$schema")" || fail "RLS schema has no runtime role"
  expected_runtime_policy="$(rehearsal_runtime_policy_name "$schema" "$table")" || fail "RLS table has no canonical runtime policy name"
  if [[ "$schema" == valorant && "$table" != _migration_ledger ]]; then
    valorant_rls_tables=$((valorant_rls_tables+1))
    [[ "$protected" == t && "$policy" == "$expected_runtime_policy" && "$roles" == "$expected_runtime_role" && "$command" == 'ALL' && "$qual" == true && "$with_check" == true ]] || fail "VALORANT table lacks its exact val_runtime RLS policy"
    valorant_policy_rows=$((valorant_policy_rows+1))
  elif [[ "$schema" == public && "$table" != _prisma_migrations ]]; then
    public_rls_tables=$((public_rls_tables+1))
    [[ "$protected" == t && "$policy" == "$expected_runtime_policy" && "$roles" == "$expected_runtime_role" && "$command" == 'ALL' && "$qual" == true && "$with_check" == true ]] || fail "Quest table lacks its exact quest_runtime RLS policy"
    public_policy_rows=$((public_policy_rows+1))
  fi
done < "$scratch/rls"
psql_query "$scratch/policy-counts" "SELECT schemaname || '|' || count(*) FROM pg_policies WHERE schemaname IN ('public','valorant') AND tablename NOT IN ('_prisma_migrations','_migration_ledger') GROUP BY schemaname ORDER BY schemaname" || fail "runtime policy coverage query failed"
public_policy_count=0; valorant_policy_count=0
while IFS='|' read -r policy_schema policy_count; do [[ "$policy_count" =~ ^[0-9]+$ ]] || fail "runtime policy count output malformed"; [[ "$policy_schema" == public ]] && public_policy_count="$policy_count"; [[ "$policy_schema" == valorant ]] && valorant_policy_count="$policy_count"; done < "$scratch/policy-counts"
(( public_policy_count > 0 && valorant_policy_count > 0 && public_policy_count == public_rls_tables && valorant_policy_count == valorant_rls_tables && public_policy_count == public_policy_rows && valorant_policy_count == valorant_policy_rows )) || fail "runtime policy coverage is incomplete or semantically inconsistent"
runtime_policy_status=verified
...
validate_failure_output() {
  ...
  declare -A failure=()
  while IFS= read -r line || [[ -n "$line" ]]; do [[ "$line" =~ ^([a-z][a-z0-9_]*)=([^[:space:]]+)$ ]] || return 1; key="${BASH_REMATCH[1]}"; value="${BASH_REMATCH[2]}"; case "$key" in result|failure_class|service_identity|endpoint_id|endpoint_sha256|pre_state|post_state|containment|unaffected_service) ;; *) return 1 ;; esac; [[ -z "${failure[$key]+x}" ]] || return 1; failure["$key"]="$value"; done < "$output"
...
for injection in bad_checksum bad_decryption wrong_ca blocked_network failed_service_health attempted_mutation_callback; do
  ...
  hook_source_sha256["$injection"]="$configured_hook_sha256"; hook_staged_sha256["$injection"]="$staged_hook_sha256_before"
  ...
  [[ "$(sha256sum "$hook" | cut -d' ' -f1)" == "$configured_hook_sha256" ]] || fail "failure injection $injection source executable changed during execution"
...
end="$(date -u +%s)"; end_utc="$(date -u -d "@$end" +%Y-%m-%dT%H:%M:%SZ)"; duration=$((end-start))
...
cp -- "$scratch/counts" "$evidence/rehearsal-schema-counts.tsv"; cp -- "$scratch/policy-counts" "$evidence/rehearsal-policy-counts.tsv"; chmod 600 "$evidence/rehearsal-schema-counts.tsv" "$evidence/rehearsal-policy-counts.tsv"
cp -- "$scratch/quest-migrations.tsv" "$evidence/rehearsal-quest-migrations.tsv"; cp -- "$scratch/valorant-migrations.tsv" "$evidence/rehearsal-valorant-migrations.tsv"; chmod 600 "$evidence/rehearsal-quest-migrations.tsv" "$evidence/rehearsal-valorant-migrations.tsv"
cp -- "$scratch/quest-migrations-before.tsv" "$evidence/rehearsal-quest-migrations-before.tsv"; cp -- "$scratch/valorant-migrations-before.tsv" "$evidence/rehearsal-valorant-migrations-before.tsv"; chmod 600 "$evidence/rehearsal-quest-migrations-before.tsv" "$evidence/rehearsal-valorant-migrations-before.tsv"
...
cp -- "$scratch/target-session-tls" "$evidence/rehearsal-target-session-tls.tsv"; cp -- "$scratch/quest-write-probe" "$evidence/rehearsal-quest-write-probe.tsv"; cp -- "$scratch/cross-schema-denial.tsv" "$evidence/rehearsal-cross-schema-denial.tsv"; chmod 600 "$evidence/rehearsal-target-session-tls.tsv" "$evidence/rehearsal-quest-write-probe.tsv" "$evidence/rehearsal-cross-schema-denial.tsv"
...
cp -- "$scratch/upload-cleanup" "$evidence/rehearsal-upload-cleanup.tsv"; chmod 600 "$evidence/rehearsal-upload-cleanup.tsv"
...
printf 'target_host=%s\ntarget_port=%s\ntarget_database_name=%s\ntarget_server_version_num=%s\ntarget_ssl_status=%s\ntarget_ssl_session_status=verified\ntarget_nonce_sha256=%s\ntarget_nonce_after_sha256=%s\nbootstrap_sql_sha256=%s\nextensions_before_status=verified\nextensions_before_count=%s\nextensions_before_inventory_sha256=%s\nsettings_before_status=verified\nsettings_before_inventory_sha256=%s\nquest_read_probe_status=verified\nquest_read_probe_count=%s\nquest_read_probe_session_user=%s\nquest_read_probe_current_user=%s\nquest_read_write_probe_status=%s\nquest_write_probe_status=%s\ncross_schema_denial_status=%s\n' "$url_host" "$url_port" "$target_identity_database" "$target_identity_server_version" "$target_identity_ssl" "$nonce_sha256" "$nonce_after_sha256" "$bootstrap_sql_sha256" "$extensions_before_count" "$extensions_before_sha256" "$settings_before_sha256" "$quest_read_probe_count" "$quest_read_probe_session_user" "$quest_read_probe_current_user" "$quest_read_write_probe_status" "$quest_write_probe_status" "$cross_schema_denial_status" >> "$observations_tmp"
printf 'quest_read_probe_application_name=%s\nquest_read_probe_backend_pid=%s\nquest_read_probe_ssl=%s\n' "$quest_read_probe_application_name" "$quest_read_probe_backend_pid" "$quest_read_probe_ssl" >> "$observations_tmp"
...
printf 'public_policy_count=%s\nvalorant_policy_count=%s\nruntime_policy_status=%s\nroles_nobypassrls_status=%s\n' "$public_policy_count" "$valorant_policy_count" "$runtime_policy_status" "$roles_nobypassrls_status" >> "$observations_tmp"
printf 'schema_counts_observations_sha256=%s\npolicy_counts_observations_sha256=%s\nquest_write_probe_observations_sha256=%s\ncross_schema_denial_observations_sha256=%s\ntarget_session_tls_observations_sha256=%s\nupload_cleanup_observations_sha256=%s\nlive_disposable_rehearsal_gate=owner_deployment_host_evidence_required\n' "$schema_counts_sha256" "$policy_counts_sha256" "$quest_write_probe_sha256" "$cross_schema_denial_sha256" "$target_session_tls_sha256" "$upload_cleanup_sha256" >> "$observations_tmp"
printf 'quest_write_probe_session_user=%s\nquest_write_probe_current_user=%s\nquest_write_probe_application_name=%s\nquest_write_probe_backend_pid=%s\nquest_write_probe_ssl=%s\ncross_schema_denial_session_user=%s\ncross_schema_denial_current_user=%s\ncross_schema_denial_application_name=%s\ncross_schema_denial_backend_pid=%s\ncross_schema_denial_ssl=%s\nupload_cleanup_status=verified\n' "$quest_write_probe_session_user" "$quest_write_probe_current_user" "$quest_write_probe_application_name" "$quest_write_probe_backend_pid" "$quest_write_probe_ssl" "$cross_schema_denial_session_user" "$cross_schema_denial_current_user" "$cross_schema_denial_application_name" "$cross_schema_denial_backend_pid" "$cross_schema_denial_ssl" >> "$observations_tmp"
...
printf 'quest_read_probe_application_name=%s\nquest_read_probe_backend_pid=%s\nquest_read_probe_ssl=%s\n' "$quest_read_probe_application_name" "$quest_read_probe_backend_pid" "$quest_read_probe_ssl" >> "$tmp"
...
printf 'public_policy_count=%s\nvalorant_policy_count=%s\nruntime_policy_status=%s\nroles_nobypassrls_status=%s\nquest_read_write_probe_status=%s\nquest_write_probe_status=%s\ncross_schema_denial_status=%s\n' "$public_policy_count" "$valorant_policy_count" "$runtime_policy_status" "$roles_nobypassrls_status" "$quest_read_write_probe_status" "$quest_write_probe_status" "$cross_schema_denial_status" >> "$tmp"
printf 'target_ssl_session_status=verified\nschema_counts_observations_sha256=%s\npolicy_counts_observations_sha256=%s\nquest_write_probe_observations_sha256=%s\ncross_schema_denial_observations_sha256=%s\ntarget_session_tls_observations_sha256=%s\nupload_cleanup_observations_sha256=%s\nlive_disposable_rehearsal_gate=owner_deployment_host_evidence_required\n' "$schema_counts_sha256" "$policy_counts_sha256" "$quest_write_probe_sha256" "$cross_schema_denial_sha256" "$target_session_tls_sha256" "$upload_cleanup_sha256" >> "$tmp"
printf 'target_server_version_num=%s\ntarget_ssl_status=%s\n' "$target_identity_server_version" "$target_identity_ssl" >> "$tmp"
printf 'quest_write_probe_session_user=%s\nquest_write_probe_current_user=%s\nquest_write_probe_application_name=%s\nquest_write_probe_backend_pid=%s\nquest_write_probe_ssl=%s\ncross_schema_denial_session_user=%s\ncross_schema_denial_current_user=%s\ncross_schema_denial_application_name=%s\ncross_schema_denial_backend_pid=%s\ncross_schema_denial_ssl=%s\nupload_cleanup_status=verified\n' "$quest_write_probe_session_user" "$quest_write_probe_current_user" "$quest_write_probe_application_name" "$quest_write_probe_backend_pid" "$quest_write_probe_ssl" "$cross_schema_denial_session_user" "$cross_schema_denial_current_user" "$cross_schema_denial_application_name" "$cross_schema_denial_backend_pid" "$cross_schema_denial_ssl" >> "$tmp"
...
REHEARSAL_TRUSTED_SIGNING_PUBLIC_KEY="$trusted_self_key" bash "$base/rehearsal/verify-rehearsal-evidence.sh" "$archive" "$evidence" >"$scratch/self-verification.log" 2>&1 || { cat "$scratch/self-verification.log" >&2; fail "generated evidence did not pass the verifier"; }
