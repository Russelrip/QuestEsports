...
Create and manage entrance fees from `/admin/tickets`. Each fee must be linked to one Event Series representing the LAN event, and an Event Series can have at most one entrance-fee setup. The purchase area appears only on that event's public page; there is no general public ticket catalog. The setup controls its venue and start time, sales window, capacity, currency, single-ticket price, pair-bundle price, accepted payment methods, and lifecycle status. For the standard offer, set the single price to LKR 500 and the pair price to LKR 800. The server always calculates totals as complete pairs plus an optional single: one ticket is LKR 500, two are LKR 800, and three are LKR 1,300.

Entrance fees can accept PayHere, manual bank transfer, cash at the entrance, or any combination selected by the admin. Bank transfer requires bank account details and a review duration. The buyer transfers the exact amount with the generated reference and uploads a receipt; QR tickets activate only after an admin approves the proof from `/admin/payments`. Cash creates a short pending reservation for use at the entrance; staff must collect the full amount and confirm it from `/admin/payments` before QR tickets activate. Cancelling or allowing an unpaid reservation to expire releases its capacity.
