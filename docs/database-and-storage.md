...
- Production backups are implemented by `ops/backup-production.sh`, which prevents overlapping runs, copies both immutable upload roots before and after the portable application-owned PostgreSQL dump of both the `public` and `valorant` schemas (when the VALORANT schema exists), encrypts the staged archive with an offline `age` recipient, uploads it through `rclone`, writes a checksum, and content-verifies both remote objects. Supabase-managed schemas/extensions are excluded and must be supplied by the target Supabase project.
