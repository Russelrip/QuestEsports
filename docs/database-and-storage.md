...
- Production backups target the live VPS PostgreSQL 17.11 Compose service, reached privately as `quest-postgres:5432` on `quest-shared`, and are implemented by `ops/backup-production.sh`. The dump runs from an ephemeral PostgreSQL 17 client container on that network, so backups need no published database port. Only host-run restores use the loopback staging overlay `ops/docker/compose.postgres-staging.yml` at `127.0.0.1:55432`, applied explicitly for that maintenance; do not apply it for backups. The script prevents overlapping runs, copies both immutable upload roots before and after the portable application-owned PostgreSQL dump of both the `public` and `valorant` schemas (when the VALORANT schema exists), encrypts the staged archive with an offline `age` recipient, uploads it through `rclone`, writes a checksum, and content-verifies both remote objects. Supabase is stale non-production context and must not be used as the live backup target.
- Configure the Google Drive remote with a project-owned OAuth desktop client in production status and the least-privilege `drive.file` scope. Do not depend on rclone's retiring shared client ID, print the rclone configuration, or store its client secret/token in Git or recovery notes.
- The database-only Windows snapshot helpers `ops/backup-paris-database-windows.ps1` and `ops/test-paris-database-backup-windows.ps1` are historical/pre-cutover or staging tooling. They do not include VPS uploads or replace the live PostgreSQL 17 off-site storage path.
- Use the systemd service/timer/failure templates in `ops/systemd/` for daily execution and failure paging. A backup is incomplete until rclone content verification succeeds for the encrypted archive and checksum off-site.
- Test restores on disposable PostgreSQL and temporary upload directories using `ops/restore-production-backup.sh`; never run a drill against the live VPS `quest-postgres` database or live upload roots.
- Follow [Backup and Disaster Recovery](./backup-and-disaster-recovery.md) for key custody, guarded retention, routine checks, isolated drills, and intentional production recovery. Follow [Secret and Infrastructure Recovery](./secret-and-infrastructure-recovery.md) for the backend `.env`, rclone/OAuth configuration, and installed infrastructure files excluded from the normal archive.
