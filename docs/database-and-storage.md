...
- Production backups target the live VPS PostgreSQL 17.11 service `quest-postgres` at `127.0.0.1:5433` and are implemented by `ops/backup-production.sh`. The host-run backup/recovery tools connect through the supported loopback staging overlay at `127.0.0.1:55432` while that service is exposed for maintenance. The script prevents overlapping runs, copies both immutable upload roots before and after the portable application-owned PostgreSQL dump of both the `public` and `valorant` schemas (when the VALORANT schema exists), encrypts the staged archive with an offline `age` recipient, uploads it through `rclone`, writes a checksum, and content-verifies both remote objects. Supabase is stale non-production context and must not be used as the live backup target.
