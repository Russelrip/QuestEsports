...
- `valorant-platform-backend/` is part of this monorepo and deployed in the same immutable release.
...
Production uses immutable Docker Compose on the Quest VPS. The `quest-prod`
project runs the frontend, Express API, migrator and private PostgreSQL 17;
`valorant-prod` runs FastAPI and its workers from the monorepo image. Applications
publish only loopback ports behind host Nginx. The stable database alias is
`quest-postgres`; no production database host port is published. The protected
release controller verifies image digests, signatures, readiness and writer
admission. See the [production runbook](./production-runbook.md).
...
Compose frontend configuration is bound to the release. After changing `SITE_MAINTENANCE_MODE`, redeploy the approved immutable release and follow the frontend-first enable/backend-first disable order in the [Production Operations Runbook](./production-runbook.md#site-maintenance-mode).
...
## Generic non-production deployment steps
...
For current Compose provisioning, Actions secrets, host-key pinning, rollback behavior, troubleshooting, updates, and reboot validation, use the [Production Operations Runbook](./production-runbook.md).
