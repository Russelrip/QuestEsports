...
Both values must be exact HTTPS origins without credentials, paths, queries, fragments, or trailing slashes. The frontend intentionally renders pages per request because Next.js script tags must receive the per-request CSP nonce generated by `proxy.ts`. Treat this as a security/performance tradeoff: keep immutable asset caching enabled and monitor server-render latency before changing the nonce architecture.
