...
| `GET` | `/api/v1/admin/valorant/leaderboard/players` | `GET /api/v1/leaderboard/players` — every player leaderboard registration, including rows the public board hides; `q`, `hidden=true` (only players hidden by staff), `page`, `per_page`. Each entry carries `hiddenAt`, `hiddenBy` (`{ id, username }`) and `hiddenReason`. Open to admins and to users with the `valorant_leaderboard` staff permission |
...
| `GET` | `/api/v1/admin/valorant/leaderboard/removals` | `GET /api/v1/leaderboard/removals` — removed registrations newest first; `q` (Riot ID or Discord username), `page`, `per_page` (max 100). Each entry has `removalId`, the row as removed, `removedAt`/`removedBy`, `restoredAt`/`restoredBy` (`{ id, username }`), and `registeredAgain`, `superseded`, `banned`, `restorable`. Removals made before migration `0017` were not kept. Open to admins and to users with the `valorant_leaderboard` staff permission |
| `POST` | `/api/v1/admin/valorant/leaderboard/removals/{removalId}/restore` | `POST /api/v1/leaderboard/removals/{removal_id}/restore` — body `{ reason }` required; re-inserts the removed row unchanged and audits `valorant.leaderboard_player.restore`. `409` when already restored (`LEADERBOARD_REMOVAL_ALREADY_RESTORED`), when the player was removed again later (`LEADERBOARD_REMOVAL_SUPERSEDED`), when the PUUID is registered again (`LEADERBOARD_PLAYER_ALREADY_REGISTERED`) when its Discord identity now belongs to another registration (`LEADERBOARD_DISCORD_ALREADY_REGISTERED`), or while the player is banned (`LEADERBOARD_PLAYER_BANNED`); `404` for an unknown removal. Open to admins and to users with the `valorant_leaderboard` staff permission |
| `POST` | `/api/v1/admin/valorant/leaderboard/players/{puuid}/hide` | `POST /api/v1/leaderboard/players/{puuid}/hide` — body `{ reason }` required (the player sees it on their profile); keeps the player off the public board, search and stats while they stay registered and connected; clears the linked player's cached profile rank; audited as `valorant.leaderboard_player.hide`. Returns `{ player, rankingsCleared }`. `409 LEADERBOARD_PLAYER_ALREADY_HIDDEN`, `404` for an unregistered player. Open to admins and to users with the `valorant_leaderboard` staff permission |
| `DELETE` | `/api/v1/admin/valorant/leaderboard/players/{puuid}/hide` | `DELETE /api/v1/leaderboard/players/{puuid}/hide` — body `{ reason }` required; puts a hidden player back on the board; audited as `valorant.leaderboard_player.unhide`. Returns `{ player }`. `409 LEADERBOARD_PLAYER_NOT_HIDDEN`. Open to admins and to users with the `valorant_leaderboard` staff permission |
| `POST` | `/api/v1/admin/valorant/leaderboard/players/{puuid}/ban` | `POST /api/v1/leaderboard/players/{puuid}/ban` — body `{ reason }` required; bans the registration's Riot account (PUUID) and Discord account and removes every registration either holds, in one transaction; audited as `valorant.leaderboard_player.ban` (Riot IDs and Discord handles only, no PUUIDs). Returns `{ ban, removed[] (each with removalId), rankingsCleared }`. `404` for an unknown player, `409 LEADERBOARD_PLAYER_ALREADY_BANNED` when both accounts are already banned. Open to admins and to users with the `valorant_leaderboard` staff permission |
| `POST` | `/api/v1/admin/valorant/leaderboard/removals/{removalId}/ban` | `POST /api/v1/leaderboard/removals/{removal_id}/ban` — the same ban, by the accounts a removal kept; also removes anything registered on either account since. `404` for an unknown removal, `409` when already banned. An account another active ban covers is left out of the new ban. Open to admins and to users with the `valorant_leaderboard` staff permission |
| `GET` | `/api/v1/admin/valorant/leaderboard/bans` | `GET /api/v1/leaderboard/bans` — bans newest first; `status` (`active` default, `lifted`, `all`), `page`, `per_page` (max 100). Each entry has `banId`, `puuid` (null when the ban covers only a Discord account), `discordBanned`, the Riot ID and Discord handle when banned, `reason`, `bannedAt`/`bannedBy`, `liftedAt`/`liftedBy`, `active`. The Discord id itself is never returned. Open to admins and to users with the `valorant_leaderboard` staff permission |
| `POST` | `/api/v1/admin/valorant/leaderboard/bans/{banId}/lift` | `POST /api/v1/leaderboard/bans/{ban_id}/lift` — body `{ reason }` required; the accounts can register again, the ban is kept as lifted; audited as `valorant.leaderboard_player.unban`. `409 LEADERBOARD_BAN_ALREADY_LIFTED`, `404` for an unknown ban. Open to admins and to users with the `valorant_leaderboard` staff permission |
| `GET` | `/api/v1/admin/valorant/leaderboard/server-checks` | `GET /api/v1/leaderboard/server-checks` — registered players the server check flags for review (`status=flagged`, default), has been told to keep (`status=cleared`), or every registration by name (`status=all`); `q` (Riot ID or Discord username), `server` (players with a match on that server, most matches there first), `sort` (`rank` highest ELO first, `rank_low`, `away`, `matches`, `recent`, `name`; players without the value last; default keeps each view's own order), `page`, `per_page` (max 100). Flagged first by share of away matches. Each entry has the player, `accountRegion`, `status`, `reasons` (`away_servers`, `account_region`), `matches`/`knownMatches`/`awayMatches`/`awayShare`, `servers` (`{ cluster, matches, home }`), `since`, `checkedAt`, `clearedAt`/`clearedBy`. The page also carries `summary` (`registered`, `checked`, `flagged`, `cleared`, and `servers`: `{ cluster, matches, players, home }` per server across every registration) and the `rule` in force. Open to admins and to users with the `valorant_leaderboard` staff permission |
| `POST` | `/api/v1/admin/valorant/leaderboard/server-checks/{puuid}/clear` | `POST /api/v1/leaderboard/server-checks/{puuid}/clear` — body `{ reason }` required; keeps a flagged player (matches before now stop counting) and audits `valorant.leaderboard_player.server_check_clear`. `409 LEADERBOARD_SERVER_CHECK_NOT_FLAGGED` when not flagged; `404` for an unregistered player. Open to admins and to users with the `valorant_leaderboard` staff permission |
| `DELETE` | `/api/v1/admin/valorant/leaderboard/server-checks/{puuid}/clear` | `DELETE /api/v1/leaderboard/server-checks/{puuid}/clear` — body `{ reason }` required; undoes a clearance and audits `valorant.leaderboard_player.server_check_reopen`. `409 LEADERBOARD_SERVER_CHECK_NOT_CLEARED` when not cleared. Open to admins and to users with the `valorant_leaderboard` staff permission |
...
Preview and submit answer `403` with "This Riot account is banned from the VALORANT leaderboard." or "This Discord account is banned from the VALORANT leaderboard." while an active leaderboard ban covers the PUUID or the caller's linked Discord account (upstream code `REGISTRATION_BANNED`).
