...
| `GET` | `/api/v1/admin/valorant/leaderboard/server-checks` | `GET /api/v1/leaderboard/server-checks` — registered players the server check flags for review (`status=flagged`, default) or has been told to keep (`status=cleared`); `q` (Riot ID or Discord username), `page`, `per_page` (max 100). Flagged first by share of away matches. Each entry has the player, `accountRegion`, `status`, `reasons` (`away_servers`, `account_region`), `matches`/`knownMatches`/`awayMatches`/`awayShare`, `servers` (`{ cluster, matches, home }`), `since`, `checkedAt`, `clearedAt`/`clearedBy`. The page also carries `summary` (`registered`, `checked`, `flagged`, `cleared`) and the `rule` in force. Open to admins and to users with the `valorant_leaderboard` staff permission |
