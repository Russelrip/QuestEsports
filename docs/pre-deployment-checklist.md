...
| Migration approval and backup | A migration-changing deploy requires `BACKEND_MIGRATION_APPROVAL_SHA` to equal the exact tested commit and creates an encrypted off-site database/upload backup before migration. | Confirm `/etc/quest-esports-backup.env`, the offline `age` recipient, the project-owned Google OAuth client and least-privilege `rclone` destination, and a recent restore drill before setting the one-release approval SHA. |
