...
| Alerts | Unhandled exceptions can be shipped through `MONITORING_WEBHOOK_URL`; readiness reports database/cache state. | **Deployment blocker:** configure a monitored webhook or equivalent platform alert and prove a test alert reaches the on-call owner. Add uptime monitoring for `/api/health/ready`. |
