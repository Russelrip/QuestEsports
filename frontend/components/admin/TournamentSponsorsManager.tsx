...
export default function TournamentSponsorsManager({ tournamentId }: { tournamentId: string }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    body.append("name", name); body.append("websiteUrl", websiteUrl); body.append("displayOrder", displayOrder);
    if (logo) body.append("logo", logo);
    try {
      await adminRequest(`/api/admin/tournaments/${tournamentId}/sponsors`, { method: "POST", body });
      setName(""); setWebsiteUrl(""); setDisplayOrder("100"); setLogo(null); setMessage("Sponsor added."); await load();
  ...
  return <Card className="p-6 sm:p-8">
    ...
    <p className="mt-1 text-sm text-slate-400">Logos are displayed in ascending order on the tournament hero.</p>
    ...
    <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-3">
        ...
        <div className="min-w-0 flex-1"><p className="truncate text-white">{item.name}</p><p className="text-xs text-slate-500">Order {item.displayOrder}</p></div>
