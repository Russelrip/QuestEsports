...
export default function AdminPaymentsManager() {
  ...
  const loadDetail = useCallback(async () => {
    ...
    setDetailError("");
    try {
      const data = await adminRequest<{ payment: PaymentDetail }>(
        `/api/admin/payments/${selectedId}`,
      );
      setSelectedPayment(data.payment);
    } catch (error) {
      setDetailError(
        error instanceof Error ? error.message : "Unable to load this payment.",
      );
    } finally {
  ...
  const paymentGroups = payments.reduce<
    Array<{ key: string; label: string; payments: PaymentSummary[] }>
  >((groups, payment) => {
    const key = `${payment.purpose}:${payment.groupId || payment.groupName}`;
    const existing = groups.find((group) => group.key === key);
    if (existing) existing.payments.push(payment);
    else groups.push({ key, label: payment.groupName, payments: [payment] });
    return groups;
  }, []);
  ...
  return (
    <AdminShell
      ...
      description="Browse transaction summaries, then open one payment to inspect evidence and perform reconciliation actions."
    >
      {selectedId ? (
        <PaymentDetailView
        ...
        <Card className="min-w-0 overflow-hidden">
          <div className="border-b border-white/10 p-5 sm:p-6">
            <div className="grid gap-4 lg:grid-cols-3">
              <Input
                ...
                placeholder="Search reference, team, or email..."
              ...
              >
                ...
                <option value="tournament_registration">
                  Tournament registration
                </option>
                <option value="ticket_order">Event ticket order</option>
          ...
          {message ? (
            <p className="p-5 text-sm text-rose-300">{message}</p>
          ) : null}
          {loading ? (
            <div className="p-5">
              <AdminTableSkeleton />
            ...
            <>
                ...
                <table className="w-full min-w-[1050px] border-collapse text-left">
                  ...
                  <tbody className="divide-y divide-white/8">
                    {paymentGroups.flatMap((group) => [
                      <tr
                        key={`group-${group.key}`}
                        className="border-y border-purple-300/15 bg-purple-400/[0.06]"
                      >
                        <td
                          colSpan={8}
                          className="px-5 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-purple-100"
                        >
                          {group.label} · {group.payments.length} transaction
                      ...
                      ...group.payments.map((payment) => (
                        <tr
                          key={payment.id}
                          className={`transition hover:bg-purple-300/[0.04] ${payment.status === "review_required" ? "bg-amber-300/[0.03]" : ""}`}
                        >
                          <td className="px-5 py-4">
                            <p className="max-w-52 break-all text-sm font-semibold text-white">
                              {payment.orderId}
                            </p>
                            {payment.paymentId ? (
                              <p className="mt-1 max-w-52 break-all text-xs text-slate-500">
                                PayHere {payment.paymentId}
                              </p>
                            ) : null}
                          </td>
                          <td className="px-5 py-4">
                          ...
                          <td className="px-5 py-4 text-xs uppercase tracking-wider text-slate-400">
                          ...
                          <td className="px-5 py-4 text-sm text-slate-400">
                            {formatAdminCompactDateTime(payment.createdAt)}
                          </td>
              ...
              <div className="flex flex-col gap-3 border-t border-white/10 px-5 py-4 text-sm text-slate-400 sm:flex-row sm:items-center sm:justify-between">
                <span>
                  {getAdminPaginationSummary(pagination, "transactions")}
                </span>
...
function PaymentDetailView({
  ...
  const openProof = async () => {
    ...
    try {
      const response = await apiFetch(
        `/api/admin/payments/${payment.id}/bank-transfer-proof`,
      );
      ...
      setActionError(
        nextError instanceof Error
          ? nextError.message
          : "Payment proof could not be opened.",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="space-y-4">
      ...
      {!loading && payment ? (
        <Card
          className={`min-w-0 overflow-hidden p-5 sm:p-6 ${payment.status === "review_required" ? "border-amber-300/30" : ""}`}
        >
          <div className="border-b border-white/10 pb-5">
            <p className="text-xs uppercase tracking-[0.2em] text-purple-200">
              {payment.purpose.replaceAll("_", " ")} · {payment.provider}
            </p>
            ...
            <p className="mt-3 break-all text-sm text-slate-400">
              ...
              {payment.paymentId ? ` · Provider ID ${payment.paymentId}` : ""}
            </p>
          </div>
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-3">
            <PaymentInfo
              ...
              rows={[
                ...
                ["Created", formatAdminCompactDateTime(payment.createdAt)],
                ["Updated", formatAdminCompactDateTime(payment.updatedAt)],
                ["Status message", payment.statusMessage || "None"],
              ]}
            />
            <PaymentInfo
              title="Customer"
              rows={
                payment.registration
                  ? [
                      ["Team", payment.registration.teamName],
                      ["Email", payment.registration.contactEmail],
                      [
                        "Slot",
                        payment.registration.assignedSlotNumber
                          ? `#${payment.registration.assignedSlotNumber}`
                          : "Not assigned",
                      ],
                      [
                        "Reserved until",
                        payment.registration.reservedUntil
                          ? formatAdminCompactDateTime(
                              payment.registration.reservedUntil,
                            )
                          : "Not reserved",
                  ...
                  : payment.ticketOrder
                    ? [
                        [
                          "Buyer",
                          `${payment.ticketOrder.firstName} ${payment.ticketOrder.lastName}`.trim(),
                        ],
                        ["Email", payment.ticketOrder.email],
                        ["Event", payment.ticketOrder.event.title],
                      ]
                    : [
                        [
                          "Email",
                          payment.merchandiseOrder?.email ||
                            "No customer email",
                        ],
                      ]
              }
              ...
              rows={[
                [
                  ...
                  payment.reconciledAt
                    ? formatAdminCompactDateTime(payment.reconciledAt)
                    : "No",
                ],
                ["Note", payment.reconciliationNote || "None"],
                ["Refund reference", payment.providerRefundId || "None"],
              ]}
            />
          </div>
          ...
          {payment.bankTransferProof ? (
            <div className="mt-5 grid gap-3 border border-white/10 bg-black/15 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">
                Private bank-transfer evidence
              </p>
              <p className="break-words text-sm text-slate-300">
                {payment.bankTransferProof.originalFilename} ·{" "}
                {Math.ceil(payment.bankTransferProof.byteSize / 1024)} KB ·{" "}
                {payment.bankTransferProof.contentType}
              </p>
              <p className="text-sm text-slate-400">
                Submitted{" "}
                {formatAdminCompactDateTime(
                  payment.bankTransferProof.submittedAt,
                )}
                {payment.bankTransferProof.reviewedAt
                  ? ` · Reviewed ${formatAdminCompactDateTime(payment.bankTransferProof.reviewedAt)}`
                  : ""}
              </p>
              {payment.bankTransferProof.rejectionReason ? (
            ...
            <div className="mt-5 grid gap-3 border border-amber-300/20 p-4">
              ...
              <div className="grid gap-2 sm:flex sm:flex-wrap">
                <Button
                  ...
                  onClick={() =>
                    void runAction(
                      `/api/admin/payments/${payment.id}/bank-transfer-review`,
                      {
                        method: "PATCH",
                ...
                <Button
                  ...
                  onClick={() =>
                    void runAction(
                      `/api/admin/payments/${payment.id}/bank-transfer-review`,
                      { method: "PATCH", json: { decision: "reject", reason } },
                    )
                  }
                >
          ...
          payment.status === "review_required" ? (
            <div className="mt-5 grid gap-3 border border-amber-300/20 p-4">
              <p className="text-sm text-amber-100">
                Verify this payment in the PayHere merchant portal before
                reconciling it here.
              </p>
              <Textarea
                value={reason}
              ...
              <Input
                ...
                placeholder="PayHere refund reference (required for refund)"
              />
              <div className="grid gap-2 sm:flex sm:flex-wrap">
                <Button
                  type="button"
                  ...
                  onClick={() =>
                    void runAction(
                      `/api/admin/payments/${payment.id}/payhere-reconciliation`,
                      {
                        method: "PATCH",
                        json: {
                          decision: "accept",
                ...
                <Button
                  type="button"
                  variant="secondary"
                  disabled={busy || !reason.trim() || !refundReference.trim()}
                  onClick={() =>
                    void runAction(
                      `/api/admin/payments/${payment.id}/payhere-reconciliation`,
                      {
          ...
          payment.status === "expired" ? (
            <div className="mt-5 border border-amber-300/20 p-4">
              <p className="mb-3 text-sm text-amber-100">
                Reopening begins a new payment window
                {payment.provider === "bank_transfer"
                  ? " and assigns the lowest available slot; the slot-tier price may change"
                  : " if tournament capacity is still available"}
                .
              </p>
              <Button
                type="button"
                ...
                onClick={() =>
                  window.confirm(
                    `Reopen expired payment ${payment.orderId}?`,
                  ) &&
                  void runAction(`/api/admin/payments/${payment.id}/reopen`, {
                    method: "POST",
                  })
                }
              >
                {busy ? "Reopening..." : "Reopen payment"}
              </Button>
            </div>
          ) : null}
...
function PaymentStatus({ value }: { value: string }) {
  const tone =
    value === "paid"
      ? "text-emerald-300"
      : ["failed", "cancelled", "charged_back", "refunded"].includes(value)
        ? "text-rose-300"
        : "text-amber-300";
  return (
