...
export default function AdminSupportThread({ conversation: initial, currentUserId, onChanged, onAssign, readError, busy: externalBusy = false }: { conversation: SupportConversation; currentUserId: string; onChanged: (conversationId: string) => void; onAssign: (id: string | null) => Promise<void>; readError?: string | null; busy?: boolean }) {
  const [conversation, setConversation] = useState(initial); const [body, setBody] = useState(""); const [screenshots, setScreenshots] = useState<File[]>([]); const [error, setError] = useState<string | null>(null); const [busy, setBusy] = useState(false);
  ...
  const send = async (event: React.FormEvent) => { event.preventDefault(); if (!body.trim()) { setError("Reply message is required."); return; } await run(async () => { const result = await (screenshots.length ? sendAdminSupportMessage(conversation.id, body.trim(), screenshots) : sendAdminSupportMessage(conversation.id, body.trim())); setConversation((current) => ({ ...current, status: result.status, messages: [...current.messages, result.message] })); setBody(""); setScreenshots([]); onChanged(conversation.id); }); };
  const changeStatus = () => void run(async () => { const next = await updateAdminSupportStatus(conversation.id, conversation.status === "RESOLVED" ? "OPEN" : "RESOLVED"); setConversation(next); onChanged(conversation.id); });
  return <div className="flex min-h-[38rem] flex-col">
    <header className="border-b border-white/8 p-5 sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><p className="text-xs uppercase tracking-[.25em] text-cyan-200/70">Private support thread</p><h2 className="mt-2 break-words text-2xl text-white">{conversation.subject}</h2><p className="mt-2 text-sm text-slate-500">{person(conversation.owner)} · opened {format(conversation.createdAt)}</p></div><div className="flex flex-wrap items-center gap-2"><Badge>{labels[conversation.status]}</Badge><Button size="sm" variant={conversation.status === "RESOLVED" ? "secondary" : "ghost"} disabled={busy || externalBusy} onClick={changeStatus}>{conversation.status === "RESOLVED" ? "Reopen" : "Resolve"}</Button></div></div>{readError ? <p role="alert" className="mt-3 text-sm text-amber-200">{readError}</p> : null}<div className="mt-5 max-w-sm"><SupportAssignmentControl conversation={conversation} currentUserId={currentUserId} busy={busy || externalBusy} onChange={(id) => void run(() => onAssign(id))} /></div></header>
