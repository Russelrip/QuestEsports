...
export default function AdminSupportThread({ conversation: initial, currentUserId, onChanged, onAssign, readError, busy: externalBusy = false }: { conversation: SupportConversation; currentUserId: string; onChanged: (conversationId: string) => void; onAssign: (id: string | null) => Promise<void>; readError?: string | null; busy?: boolean }) {
  ...
  const send = async (event: React.FormEvent) => { event.preventDefault(); if (!body.trim()) { setError("Reply message is required."); return; } await run(async () => { const result = await sendAdminSupportMessage(conversation.id, body.trim()); setConversation((current) => ({ ...current, status: result.status, messages: [...current.messages, result.message] })); setBody(""); onChanged(conversation.id); }); };
