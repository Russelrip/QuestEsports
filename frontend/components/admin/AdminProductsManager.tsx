...
export default function AdminProductsManager() {
  ...
  return <AdminShell title="Products" description="Publish merchandise, variants, prices, images, made-to-order rules, and stock.">
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      ...
      <div className="grid gap-4">{products.map((product) => <Card key={product.id} className="p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.2em] text-purple-200">{product.status} · {product.currency}</p><h3 className="mt-2 text-xl text-white">{product.name}</h3><p className="mt-2 text-sm text-slate-400">{product.variants.length} variants · {product.images.length} images</p></div><div className="flex gap-2"><Button variant="secondary" onClick={() => edit(product)}>Edit</Button>{product.status !== "archived" ? <Button variant="danger" onClick={async () => { if (!confirm(`Archive ${product.name}?`)) return; try { await adminRequest(`/api/admin/products/${product.id}`, { method: "DELETE" }); await load(); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to archive product."); } }}>Archive</Button> : null}</div></div></Card>)}</div>
    </div>
  </AdminShell>;
}
