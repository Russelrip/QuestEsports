...
export default function AdminSupportManager() {
  ...
  const assign = async (id: string | null) => {
    ...
    try {
      const result = await assignSupportConversation(selectedId, id);
      if (selectedId === result.id) setSelected(result);
      await load();
    } finally {
