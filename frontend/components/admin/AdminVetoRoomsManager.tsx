...
import { type VetoCatalog, type VetoRoom, type VetoStep, vetoRequest } from "@/lib/veto";
...
type IssuedTokens = { team1: string; team2: string; viewer: string | null };
...
export default function AdminVetoRoomsManager() {
  ...
  const rotate = async (role: "team_1" | "team_2" | "viewer") => {
    if (!selected) return;
    setBusy(role);
    try {
  ...
  const shareUrl = (token: string) => `${window.location.origin}/veto/${selected?.code}#access=${token}`;
  const copyLink = async (token: string) => { await navigator.clipboard.writeText(shareUrl(token)); setMessage("Private room link copied."); };

  return (
    <AdminShell title="Veto Rooms" description="Create and run animated Valorant toss, map veto, and starting-side rooms from desktop or mobile." actions={<div className="flex gap-2"><Button variant="secondary" onClick={() => setShowLibrary((value) => !value)}>Maps & presets</Button><Button onClick={() => { setShowCreate(true); setWizardStep(1); }}>New veto room</Button></div>}>
      ...
      {showCreate ? <Card className="p-5 sm:p-8">
        ...
        <div className="mt-6">
          {wizardStep === 1 ? <div className="grid gap-3 sm:grid-cols-4">{(["bo1", "bo3", "bo5", "custom"] as const).map((format) => <button type="button" key={format} onClick={() => chooseFormat(format)} className={`min-h-28 border p-5 text-left transition hover:-translate-y-1 ${form.format === format ? "border-purple-300/50 bg-purple-400/12" : "border-white/10 bg-white/[.025]"}`}><span className="text-2xl font-black uppercase text-white">{format}</span><span className="mt-2 block text-xs text-slate-400">{format === "custom" ? "Custom rule sequence" : `Best of ${format.slice(2)}`}</span></button>)}</div> : null}
      ...
      {showLibrary ? <div className="grid gap-5 xl:grid-cols-3">
        <Card className="p-5"><p className="text-xs uppercase tracking-[.18em] text-purple-200">Map catalog</p><h2 className="mt-2 text-xl text-white">Add a map</h2><div className="mt-4 grid gap-3"><Input placeholder="Map name" value={mapForm.name} onChange={(event) => setMapForm({ ...mapForm, name: event.target.value })} /><label className="text-sm text-slate-400">Accent color<Input className="mt-2" type="color" value={mapForm.accentColor} onChange={(event) => setMapForm({ ...mapForm, accentColor: event.target.value })} /></label><Button disabled={busy === "map"} onClick={() => void createMap()}>Add map</Button></div><div className="mt-5 flex flex-wrap gap-2">{catalog?.maps.map((map) => <span key={map.slug} className="border border-white/8 px-2 py-1 text-xs text-slate-300" style={{ borderLeftColor: map.accentColor }}>{map.name}</span>)}</div></Card>
        <Card className="p-5"><p className="text-xs uppercase tracking-[.18em] text-purple-200">Map pool</p><h2 className="mt-2 text-xl text-white">Save a pool version</h2><div className="mt-4 grid gap-3"><Input placeholder="Pool name" value={poolName} onChange={(event) => setPoolName(event.target.value)} /><div className="grid max-h-80 gap-2 overflow-y-auto">{catalog?.maps.map((map) => <label key={map.slug} className="flex items-center gap-3 border border-white/8 p-3 text-sm text-slate-300"><input type="checkbox" checked={Boolean(map.id && poolMapIds.includes(map.id))} onChange={(event) => map.id && setPoolMapIds((current) => event.target.checked ? [...current, map.id!] : current.filter((id) => id !== map.id))} />{map.name}</label>)}</div><Button disabled={busy === "pool" || !poolMapIds.length} onClick={() => void createPool()}>Save new pool version</Button><p className="text-xs text-slate-500">{form.tournamentId ? "Saved for the selected tournament." : "Saved organization-wide."}</p></div></Card>
        <Card className="p-5"><p className="text-xs uppercase tracking-[.18em] text-purple-200">Rule builder</p><h2 className="mt-2 text-xl text-white">Save a veto preset</h2><div className="mt-4 grid gap-3"><Input placeholder="Preset name" value={presetName} onChange={(event) => setPresetName(event.target.value)} /><Select value={presetFormat} onChange={(event) => selectPresetBase(event.target.value as VetoRoom["format"])}>{["bo1", "bo3", "bo5", "custom"].map((format) => <option key={format} value={format}>{format.toUpperCase()}</option>)}</Select>{presetSteps.length === 0 ? <Button variant="secondary" onClick={() => selectPresetBase(presetFormat)}>Load a base sequence</Button> : <div className="grid max-h-96 gap-2 overflow-y-auto">{presetSteps.map((step, index) => <div key={index} className="grid grid-cols-[1fr_.7fr_64px_auto] gap-2 border border-white/8 p-2"><Select value={step.kind} onChange={(event) => setPresetSteps((current) => current.map((entry, itemIndex) => itemIndex === index ? { ...entry, kind: event.target.value as VetoStep["kind"] } : entry))}><option value="ban">Ban</option><option value="pick">Pick</option><option value="decider">Decider</option><option value="side">Side</option></Select><Select disabled={step.kind === "decider"} value={step.actor || ""} onChange={(event) => setPresetSteps((current) => current.map((entry, itemIndex) => itemIndex === index ? { ...entry, actor: (event.target.value || null) as VetoStep["actor"] } : entry))}><option value="">System</option><option value="A">A</option><option value="B">B</option></Select><Input aria-label="Series map" type="number" min="1" value={step.seriesIndex || ""} onChange={(event) => setPresetSteps((current) => current.map((entry, itemIndex) => itemIndex === index ? { ...entry, seriesIndex: event.target.value ? Number(event.target.value) : null } : entry))} /><Button size="sm" variant="danger" onClick={() => setPresetSteps((current) => current.filter((_, itemIndex) => itemIndex !== index))}>×</Button></div>)}</div>}<div className="flex gap-2"><Button size="sm" variant="secondary" onClick={() => setPresetSteps((current) => [...current, { kind: "ban", actor: "A", seriesIndex: null }])}>Add step</Button><Button size="sm" disabled={busy === "preset" || !presetSteps.length} onClick={() => void createPreset()}>Save preset version</Button></div></div></Card>
      </div> : null}
      ...
      <div className="grid min-w-0 gap-5 xl:grid-cols-[340px_minmax(0,1fr)]">
        ...
        <div className="grid min-w-0 gap-5">{selected ? <>
          <Card className="p-5"><div className="flex flex-wrap gap-2"><Link href={`/veto/${selected.code}`} target="_blank" className={buttonClassName({ variant: "secondary", size: "sm" })}>Open live room</Link>{selected.status === "draft" ? <Button size="sm" disabled={Boolean(busy)} onClick={() => void command("open")}>Open room</Button> : null}{selected.teamOrderMethod === "staff_assignment" && ["draft", "open"].includes(selected.status) ? <><Button size="sm" variant="secondary" onClick={() => void command("assign-team-a", { teamASlot: 1 })}>Slot 1 is A</Button><Button size="sm" variant="secondary" onClick={() => void command("assign-team-a", { teamASlot: 2 })}>Slot 2 is A</Button></> : null}{["open", "toss_complete"].includes(selected.status) ? <Button size="sm" disabled={Boolean(busy)} onClick={() => void command("start")}>Start</Button> : null}{selected.status === "open" ? <Button size="sm" variant="secondary" disabled={Boolean(busy)} onClick={() => void command("start", { force: true })}>Force start</Button> : null}{selected.status === "toss_pending" && selected.toss.method === "manual" && !selected.toss.result ? <Button size="sm" variant="secondary" disabled={Boolean(busy)} onClick={() => void recordPhysicalToss()}>Record physical toss</Button> : null}{["in_progress", "completed"].includes(selected.status) ? <Button size="sm" variant="secondary" disabled={Boolean(busy)} onClick={() => void command("rewind", { targetStep: Math.max(0, selected.currentStep - 1) })}>Undo last step</Button> : null}<Button size="sm" variant="ghost" disabled={Boolean(busy)} onClick={() => void command("reset")}>Reset</Button><Button size="sm" variant="danger" disabled={Boolean(busy)} onClick={() => void command("cancel")}>Cancel</Button></div>
            <div className="mt-5 grid gap-3 sm:grid-cols-3">{(["team_1", "team_2", "viewer"] as const).map((role) => { const token = role === "team_1" ? issued?.team1 : role === "team_2" ? issued?.team2 : issued?.viewer; return <div key={role} className="border border-white/8 bg-black/20 p-3"><p className="text-xs uppercase text-slate-500">{formatLabel(role)} link</p><div className="mt-3 flex flex-wrap gap-2">{token ? <Button size="sm" onClick={() => void copyLink(token)}>Copy</Button> : null}<Button size="sm" variant="ghost" disabled={Boolean(busy)} onClick={() => void rotate(role)}>Rotate</Button></div></div>; })}</div>
          </Card>
          <VetoRoomView code={selected.code} />
