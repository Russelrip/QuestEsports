...
// Matches the backend: live rooms are cancelled first so a linked match gets its status back.
const DELETABLE_VETO_STATUSES: string[] = ["draft", "cancelled", "completed"];
...
export default function AdminVetoRoomsManager() {
  ...
  useEffect(() => {
    const queryKey = `${queryRoomId}|${queryMatchId}|${queryTournamentId}`;
    if (!roomsLoaded || appliedQueryKey.current === queryKey) return;
  ...
  const deleteRoom = async () => {
    if (!selected) return;
    const linked = selected.status === "completed" && selected.match ? " Its result will also disappear from the match page." : "";
    if (!window.confirm(`Delete ${selected.title} (${selected.code})? The room, its picks and bans, and every access link are removed permanently.${linked}`)) return;
    setBusy("delete"); setMessage("");
    try {
      await vetoRequest(`/api/v1/admin/veto-rooms/${selected.id}`, { method: "DELETE" });
      setRooms((current) => current.filter((room) => room.id !== selected.id));
      setSelectedId(null);
      setMessage(`${selected.title} deleted.`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "Could not delete the room."); }
    finally { setBusy(""); }
  };
  ...
  return (
    <AdminShell title="Veto Rooms" description="Create and run animated Valorant toss, map veto, and starting-side rooms from desktop or mobile." actions={<div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={() => setShowLibrary((value) => !value)}>Maps & presets</Button><Button onClick={() => { setExistingRoomId(null); setShowCreate(true); setWizardStep(1); }}>New veto room</Button></div>}>
      ...
      {showLibrary ? <Card className="mt-5 p-5"><p className="text-xs uppercase tracking-[.18em] text-purple-200">Map availability</p><p className="mt-2 text-sm text-slate-400">Inactive maps remain in the catalog for management but cannot be added to new pool versions.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{catalog?.maps.map((map) => { const active = map.isActive !== false; const mapBusy = busy === `map:${map.id}`; return <div key={`availability-${map.slug}`} className="flex items-center gap-3 border border-white/8 p-3" style={{ borderLeftColor: map.accentColor, borderLeftWidth: 3 }}><div className="h-14 w-20 shrink-0 bg-cover bg-center" style={{ backgroundColor: map.accentColor, backgroundImage: vetoMapArtwork(map) ? `url(${vetoMapArtwork(map)})` : undefined }} aria-label={`${map.name} artwork`} /><div className="min-w-0 flex-1"><p className="truncate text-sm text-white">{map.name}</p><Badge className={active ? "border-emerald-300/20 bg-emerald-400/10 text-emerald-100" : "border-amber-300/20 bg-amber-400/10 text-amber-100"}>{active ? "Active" : "Inactive"}</Badge></div><Button size="sm" variant={active ? "danger" : "secondary"} disabled={Boolean(busy) || mapBusy || !map.id} onClick={() => void toggleMapAvailability(map)}>{mapBusy ? "Saving…" : active ? "Disable" : "Enable"}</Button></div>; })}</div></Card> : null}
