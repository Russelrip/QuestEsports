  ...
  return (
    <Card className="p-6 sm:p-8">
      <div className="mb-6">
        ...
        <h3 className="mt-2 text-2xl text-white">Challonge Bracket Manager</h3>
        <p className="mt-2 text-sm leading-6 text-slate-400">The public page embeds Challonge directly and does not poll its REST API. Only deliberate admin actions below consume the 500-request monthly allowance. Credentials remain on the API server.</p>
      </div>
      {loading ? <AdminTableSkeleton rows={4} /> : <div className="grid gap-6">
        <section className="grid gap-4 border border-white/8 bg-white/5 p-4">
          ...
          <label className="flex items-center gap-3 text-sm text-slate-200"><input type="checkbox" checked={enabled} onChange={(event) => setEnabled(event.target.checked)} disabled={Boolean(busy)} />Use the public Challonge bracket on the tournament page</label>
        ...
        {integration?.editor ? <>
          <section className="grid gap-4 border border-white/8 p-4">
...
function ParticipantEditor({ participant, tournamentId, busy, runAction, onUpdated, onDeleted, setMessage }: { participant: EditorParticipant; tournamentId: string; busy: string; runAction: (key: string, action: () => Promise<void>) => Promise<void>; onUpdated: (participant: EditorParticipant) => void; onDeleted: (id: string) => void; setMessage: (message: string) => void }) {
  ...
  return <div className="grid gap-2 border border-white/8 bg-white/5 p-3 sm:grid-cols-[minmax(0,1fr)_100px_auto_auto]"><Input aria-label={`${participant.name} name`} value={name} onChange={(event) => setName(event.target.value)} disabled={Boolean(busy)} /><Input aria-label={`${participant.name} seed`} type="number" min="1" value={seed} onChange={(event) => setSeed(event.target.value)} disabled={Boolean(busy)} /><Button type="button" size="sm" variant="secondary" onClick={() => void update()} disabled={Boolean(busy) || !name.trim()}>{busy === `participant-${participant.id}` ? "Working..." : "Update"}</Button><Button type="button" size="sm" variant="danger" onClick={() => void remove()} disabled={Boolean(busy)}>Remove</Button></div>;
}
