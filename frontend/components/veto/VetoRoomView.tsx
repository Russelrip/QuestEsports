...
export default function VetoRoomView({ code }: { code: string }) {
  ...
  return (
    <div className="veto-stage grid min-w-0 gap-5">
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_auto_1fr] md:items-stretch">
        {room.participants.map((participant, index) => (
          <div key={participant.id} className={`${index === 1 ? "md:col-start-3" : ""} border border-white/10 bg-[#11131c] p-5`} style={{ borderTopColor: participant.accentColor }}>
            ...
            <p className="mt-4 text-xs uppercase tracking-[0.16em] text-slate-400">{participant.ready ? "Ready" : "Waiting"}</p>
        ...
        <Card className="relative overflow-hidden p-6 sm:p-8">
          <div className="grid gap-7 lg:grid-cols-[.75fr_1.25fr] lg:items-center">
            ...
            <div><p className="text-xs uppercase tracking-[0.2em] text-purple-200">Pre-match toss</p><h2 className="mt-2 text-2xl text-white">{room.toss.winnerSlot ? `${room.participants[room.toss.winnerSlot - 1]?.displayName} won the toss` : "Choose Heads or Tails"}</h2>
              ...
              {canChooseOrder ? <div className="mt-5"><p className="mb-3 text-sm text-slate-300">The winner chooses their veto position.</p><div className="grid grid-cols-2 gap-3"><Button disabled={Boolean(busy)} onClick={() => mutate("team-a", `/api/v1/veto-rooms/${room.code}/team-a`, { choice: "A" })}>Choose Team A</Button><Button disabled={Boolean(busy)} onClick={() => mutate("team-b", `/api/v1/veto-rooms/${room.code}/team-a`, { choice: "B" })}>Choose Team B</Button></div></div> : null}
      ...
      <Card className="p-5 sm:p-7">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs uppercase tracking-[0.2em] text-purple-200">Veto progress</p><h2 className="mt-2 text-2xl text-white">{room.status === "in_progress" && room.currentAction ? `${currentParticipant?.displayName || "System"} · ${label(room.currentAction.kind)}` : room.status === "completed" ? "Veto complete" : "Waiting to begin"}</h2></div>{countdown !== null ? <div className={`min-w-28 border px-4 py-3 text-center ${countdown <= 0 ? "border-rose-400/40 bg-rose-500/10 text-rose-200" : "border-white/10 bg-black/20 text-white"}`}><p className="text-[9px] uppercase tracking-[0.2em]">{countdown <= 0 ? "Overdue" : "Turn time"}</p><p className="mt-1 text-2xl font-black tabular-nums">{Math.max(0, countdown)}s</p></div> : null}</div>
      ...
      {room.currentAction?.kind === "side" && actionAllowed ? (
        <Card className="veto-side-reveal p-6 text-center sm:p-8"><p className="text-xs uppercase tracking-[0.2em] text-purple-200">Starting side · Map {room.currentAction.seriesIndex}</p><h2 className="mt-3 text-3xl text-white">Choose Attack or Defense</h2><div className="mx-auto mt-6 grid max-w-xl gap-3 sm:grid-cols-2"><Button className="h-16" disabled={Boolean(busy)} onClick={() => mutate("attack", `/api/v1/veto-rooms/${room.code}/actions`, { side: "attack" })}>Attack</Button><Button className="h-16" disabled={Boolean(busy)} onClick={() => mutate("defense", `/api/v1/veto-rooms/${room.code}/actions`, { side: "defense" })}>Defense</Button></div></Card>
      ) : null}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {room.maps.map((map) => {
          ...
          return <button key={map.slug} type="button" disabled={!selectable || Boolean(busy)} onClick={() => mutate(`map-${map.slug}`, `/api/v1/veto-rooms/${room.code}/actions`, { mapSlug: map.slug })} className={`veto-map-card group relative min-h-48 overflow-hidden border text-left transition ${action?.kind === "ban" ? "veto-map-card--banned border-rose-400/25" : action ? "veto-map-card--picked border-emerald-400/35" : selectable ? "border-purple-300/30 hover:-translate-y-1 hover:border-purple-200/70" : "border-white/10"}`} style={{ background: map.artworkUrl ? undefined : `radial-gradient(circle at 75% 15%, ${map.accentColor}55, transparent 35%), linear-gradient(145deg, #191b28, #090a10)` }}>
