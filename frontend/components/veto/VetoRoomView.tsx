...
export default function VetoRoomView({ code, onRoomChange }: { code: string; onRoomChange?: (room: VetoRoom) => void }) {
  ...
  // Focus moves into the confirmation, Tab stays inside it, Escape closes it,
  // and focus returns to the map or button that opened it.
  const decisionDialogRef = useDialogFocus({ open: Boolean(pendingDecision), onClose: () => setPendingDecision(null) });
