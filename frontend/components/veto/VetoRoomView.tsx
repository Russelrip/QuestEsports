...
// A gentle 3D tilt that follows the mouse over a selectable map card.
...
export default function VetoRoomView({ code, onRoomChange }: { code: string; onRoomChange?: (room: VetoRoom) => void }) {
  ...
  useGSAP(() => {
    ...
    if (!revealingToss || !coin || prefersReducedMotion()) return;
    coinFlip(coin);
  }, { scope: rootRef, dependencies: [revealingToss] });
  ...
  return (
    <div ref={rootRef} className={`veto-stage grid min-w-0 gap-5 ${isCaster ? "veto-stage--caster" : ""}`}>
      ...
      {(room.status === "toss_pending" || room.status === "toss_complete") ? (
        <Card data-veto-panel className="relative overflow-hidden p-6 sm:p-8">
          <div className="grid gap-7 lg:grid-cols-[.75fr_1.25fr] lg:items-center">
            <div className="text-center [perspective:700px]">
              <div data-veto-coin className="veto-coin" aria-label={room.toss.result ? `Coin result ${room.toss.result}` : "Coin toss pending"}><span>{room.toss.result && !revealingToss ? room.toss.result === "heads" ? "H" : "T" : "?"}</span></div>
              <div className="veto-coin-shadow mx-auto mt-3" aria-hidden="true" />
              <p className="mt-3 text-xs uppercase tracking-[0.2em] text-slate-500">{room.toss.result ? `${label(room.toss.result)} wins` : `${room.participants[room.toss.callerSlot - 1]?.displayName} calls`}</p>
            </div>
