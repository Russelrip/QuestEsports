...
function ParticipantMark({ name, logoUrl, accentColor }: { name: string; logoUrl?: string | null; accentColor: string }) {
  ...
  return <div className="grid size-12 shrink-0 place-items-center overflow-hidden rounded-xl border text-sm font-black text-white" style={{ borderColor: `${accentColor}88`, backgroundColor: `${accentColor}22` }}>
    {resolvedLogoUrl && !logoFailed ? <Image src={resolvedLogoUrl} alt="" width={48} height={48} unoptimized className="size-full object-contain" onError={() => setLogoFailed(true)} /> : <span aria-label={`${name} initials`}>{initials(name)}</span>}
  </div>;
}
...
export default function VetoRoomView({ code, onRoomChange }: { code: string; onRoomChange?: (room: VetoRoom) => void }) {
  ...
  return (
    <div className={`veto-stage grid min-w-0 gap-5 ${isCaster ? "veto-stage--caster" : ""}`}>
      <Card className="overflow-hidden border-white/10 bg-[#090a11] p-0">
        <div className="relative overflow-hidden px-5 py-6 sm:px-8 sm:py-8">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="flex flex-wrap gap-2"><Badge>{room.format.toUpperCase()}</Badge><div aria-live="polite" aria-atomic="true"><Badge>{label(room.status)}</Badge></div><Badge>{roleLabel}</Badge>{room.tournament ? <Badge>{room.tournament.title}</Badge> : null}</div>
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_auto_1fr] md:items-stretch">
        {room.participants.map((participant, index) => (
          <div key={participant.id} className={`${index === 1 ? "md:col-start-3" : ""} border border-white/10 bg-[#11131c] p-5`} style={{ borderTopColor: participant.accentColor }}>
            <div className="flex items-start justify-between gap-4"><div className="flex min-w-0 items-center gap-3"><ParticipantMark name={participant.displayName} logoUrl={participant.logoUrl} accentColor={participant.accentColor} /><div className="min-w-0"><p className="text-[10px] uppercase tracking-[0.2em] text-slate-500">Slot {participant.slot}{participant.team ? ` · Team ${participant.team}` : ""}</p><h2 className="mt-2 truncate text-xl text-white">{participant.displayName}</h2></div></div><span className={`size-3 shrink-0 rounded-full ${participant.ready ? "bg-emerald-400 shadow-[0_0_18px_#34d399]" : "bg-slate-700"}`} /></div>
      ...
      <Card className={`p-5 sm:p-7 ${isPremier ? "border-cyan-300/25 bg-[#0b111b]" : ""} ${room.access.kind === "team" && actionAllowed ? "border-cyan-300/40 bg-cyan-400/[.06]" : ""}`}>
      ...
      {isPremier ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3" aria-label="Premier map selection">{room.maps.map((map) => { const action = selectedBySlug.get(map.slug) as VetoAction | undefined; const actionKind = action?.kind || null; const selectable = !action && !getPremierMapPresentation(actionKind, null, actionAllowed && room.currentAction?.kind === "ban").disabled; const artworkUrl = resolveImageUrl(map.artworkUrl); const bannedBy = action?.actorSlot ? room.participants.find((participant) => participant.slot === action.actorSlot)?.team : null; const presentation = getPremierMapPresentation(actionKind, bannedBy || null, actionAllowed && room.currentAction?.kind === "ban"); const locked = presentation.locked; return <button key={map.slug} type="button" disabled={!selectable || Boolean(busy)} aria-label={`${map.name}${action?.kind === "ban" ? `, banned by Team ${bannedBy || "a team"}` : locked ? ", map locked" : ""}`} onClick={() => selectable && setPendingDecision({ kind: "ban", mapSlug: map.slug, mapName: map.name, revision: room.revision, seriesIndex: null })} className={`group relative min-h-56 overflow-hidden border text-left transition ${action?.kind === "ban" ? "border-rose-400/30 grayscale" : locked ? "border-amber-300 ring-2 ring-amber-300/30" : selectable ? "border-cyan-300/40 hover:-translate-y-1 hover:border-cyan-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300" : "border-white/10"}`} style={{ background: artworkUrl ? undefined : `radial-gradient(circle at 75% 15%, ${map.accentColor}66, transparent 35%), linear-gradient(145deg, #191b28, #090a10)` }}>{artworkUrl ? <Image src={artworkUrl} alt="" fill sizes="(min-width: 1280px) 33vw, (min-width: 640px) 50vw, 100vw" unoptimized className="absolute inset-0 size-full object-cover opacity-70 transition duration-500 group-hover:scale-105" onError={(event) => { const image = event.currentTarget; if (image.dataset.fallbackApplied === "true") image.style.display = "none"; else { image.dataset.fallbackApplied = "true"; image.src = "/images/logo.png"; } }} /> : null}<div className="absolute inset-0 bg-gradient-to-t from-black via-black/35 to-transparent" />{action?.kind === "ban" ? <div className="absolute inset-0 grid place-items-center bg-rose-950/60"><span className="border-2 border-rose-200/70 px-3 py-2 text-xs font-black uppercase tracking-[0.18em] text-rose-100">{bannedBy ? `Banned - Team ${bannedBy}` : "Banned"}</span></div> : null}{locked ? <div className="absolute right-4 top-4 bg-amber-300 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.18em] text-slate-950">Map locked</div> : null}<div className="relative flex min-h-56 flex-col justify-end p-5"><p className="text-[10px] uppercase tracking-[0.2em] text-slate-300">{action?.kind === "ban" ? "Banned" : locked ? "Map locked" : selectable ? "Select to ban" : "Available"}</p><h3 className="mt-2 text-2xl font-black uppercase text-white">{map.name}</h3></div></button>; })}</div> : null}
