...
export default function RefundPolicyContent() {
  return <Section className="pt-6"><div className="grid gap-6"><Card className="p-6 sm:p-8"><p className="text-xs uppercase tracking-[0.28em] text-cyan-200/80">Last Updated</p><h2 className="mt-3 text-3xl text-white">July 13, 2026</h2><p className="mt-4 text-sm leading-7 text-slate-300">This policy covers customized merchandise and paid tournament registration on questesports.lk.</p></Card>{sections.map((section) => <Card key={section.title} className="p-6 sm:p-8"><h3 className="text-2xl text-white">{section.title}</h3><p className="mt-4 text-sm leading-7 text-slate-300">{section.text}</p></Card>)}<Card className="p-6 sm:p-8"><h3 className="text-2xl text-white">Contact us</h3><p className="mt-4 text-sm leading-7 text-slate-300">Send order or payment questions to <a className="text-cyan-200" href="mailto:questesports.lk@gmail.com">questesports.lk@gmail.com</a> or use the <Link className="text-cyan-200" href="/contact">contact page</Link>.</p></Card></div></Section>;
}
