...
export default function SupportInbox({ conversationId }: { conversationId?: string }) {
  ...
  const loadList = useCallback(async () => {
    try { const result = await listSupportConversations(); setItems(result.items); setListError(null); }
    catch (error) { setListError(error instanceof Error ? error.message : "Inbox could not be loaded."); }
    finally { setListLoading(false); }
  }, []);

  const loadThread = useCallback(async () => {
    if (!conversationId) return;
    setThreadLoading(true); setThreadError(null); setReadError(null);
    try {
      const result = await getSupportConversation(conversationId);
      setConversation(result);
      try { await markSupportConversationRead(conversationId); await loadList(); }
      catch (error) { setReadError(error instanceof Error ? error.message : "Read status could not be saved."); }
    } catch (error) { setConversation(null); setThreadError(error instanceof Error ? error.message : "Conversation could not be loaded."); }
  ...
  const create = async ({ subject, body }: { subject: string; body: string }) => {
    ...
    try { const next = await createSupportConversation(subject, body); showToast({ title: "Message sent", description: "Your support conversation is ready.", tone: "success" }); await loadList(); window.history.pushState({}, "", `/support/${next.id}`); setConversation(next); return true; }
    catch (error) { setCreateError(error instanceof Error ? error.message : "Message could not be sent. Try again."); return false; }
