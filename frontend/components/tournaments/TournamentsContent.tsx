...
function TournamentCard({ tournament }: { tournament: Tournament }) {
  ...
  return <Link href={`/tournaments/${tournament.slug}`} className="group relative flex h-full flex-col overflow-hidden border border-white/10 bg-[#11131c]/95 shadow-[0_18px_45px_rgba(0,0,0,0.26)] transition duration-300 hover:-translate-y-1 hover:border-white/25 hover:bg-[#151722] hover:shadow-[0_24px_65px_rgba(0,0,0,0.42)] motion-reduce:hover:translate-y-0">
    <span className="pointer-events-none absolute inset-x-0 top-0 z-10 h-px bg-gradient-to-r from-transparent via-cyan-300/70 to-transparent opacity-60 transition group-hover:opacity-100" />
    <div className="relative aspect-[4/3] overflow-hidden bg-black/40"><TournamentBannerImage bannerUrl={tournament.bannerUrl} title={tournament.title} rounded={false} showFallbackTitle={false} className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.025]" /><span className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#11131c]/30 via-transparent to-white/[0.02]" /></div>
    <div className="relative border-t border-white/10 bg-[linear-gradient(135deg,rgba(31,34,48,0.98),rgba(20,22,32,0.98))] px-4 py-4 before:absolute before:bottom-4 before:left-0 before:top-4 before:w-0.5 before:bg-cyan-300/65"><p className="text-[9px] font-semibold uppercase tracking-[0.24em] text-cyan-200/75">{tournament.gameCategory?.displayName || tournament.game}</p><h3 className="mt-1.5 line-clamp-2 min-h-10 text-sm font-bold uppercase leading-5 text-white transition group-hover:text-cyan-50">{tournament.title}</h3></div>
    <dl className="grid flex-1 grid-cols-2 border-t border-white/8 bg-white/[0.025] text-xs"><Meta label="Organizer" value={tournament.organizer} /><Meta label="Location" value={tournament.location} /><Meta label="Registration Closing Date" value={formatTournamentDate(tournament.registrationDeadline, tournament.registrationDeadlineStatus)} /><Meta label="Event Start Date" value={formatTournamentDate(tournament.startDate, tournament.startDateStatus)} /></dl>
    <div className="flex items-center justify-between gap-3 border-t border-white/10 bg-black/20 px-4 py-3 text-[9px] font-bold uppercase tracking-[0.16em]"><span className="flex items-center gap-2 text-white/65 transition group-hover:text-white">View Tournament <span aria-hidden="true" className="text-sm leading-none text-cyan-300 transition group-hover:translate-x-0.5">→</span></span><span className={`text-right ${statusClassName}`}>{statusLabel}</span></div>
  </Link>;
}

function Meta({ label, value }: { label: string; value: string }) { return <div className="min-w-0 border-b border-r border-white/8 px-4 py-3.5 even:border-r-0"><dt className="text-[8px] font-semibold uppercase tracking-[0.2em] text-slate-500">{label}</dt><dd className="mt-1.5 line-clamp-1 text-[11px] font-semibold text-slate-200">{value}</dd></div>; }
