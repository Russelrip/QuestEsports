...
function TournamentCard({ tournament }: { tournament: Tournament }) {
  ...
  return <Link href={`/tournaments/${tournament.slug}`} className="group relative flex h-full flex-col overflow-hidden border border-white/10 bg-[#0d0c13]">
    ...
    <div className="bg-[#0d0c13] px-4 py-4"><p className="text-[9px] font-semibold uppercase tracking-[0.24em] text-cyan-200/80">{tournament.gameCategory?.displayName || tournament.game}</p><h3 className="mt-1.5 line-clamp-2 min-h-10 text-sm font-bold uppercase leading-5 text-white transition-colors group-hover:text-cyan-200">{tournament.title}</h3></div>
