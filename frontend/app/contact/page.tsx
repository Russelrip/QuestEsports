...
export default function ContactPage() {
  return (
    <PageLayout title="Contact Us" description={defaultPageDescriptions.contact}>
      ...
      <Section>
        <div className="mb-6"><p className="text-xs uppercase tracking-[0.28em] text-cyan-200">Recruitment</p><h2 className="mt-3 text-3xl text-white">Choose the application that fits your roster.</h2></div>
        ...
        ].map(([title, description, type]) => <Link key={type} href={`/join?type=${type}`}><Card className="h-full p-6 transition hover:-translate-y-1 hover:border-cyan-300/35"><h3 className="text-2xl text-white">{title}</h3><p className="mt-3 text-sm leading-7 text-slate-400">{description}</p><p className="mt-6 text-sm font-semibold text-cyan-200">Start application →</p></Card></Link>)}</div>
      </Section>
      <JoinQuestSection />
    </PageLayout>
  );
