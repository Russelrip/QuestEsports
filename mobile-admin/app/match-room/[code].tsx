...
import { clearDraftAfterSuccess } from "@/drafts";
...
export default function MatchRoomScreen() {
  ...
  const run = async (key: string, path: string, method: "POST" | "PATCH", body: unknown = {}): Promise<boolean> => {
    setBusy(key);
    try { await apiRequest(path, { method, ...jsonBody(body) }); await load(); return true; }
    catch (caught) { setError(caught instanceof Error ? caught.message : "Action failed."); return false; }
    finally { setBusy(""); }
  };
