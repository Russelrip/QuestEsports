...
-- `id` and `updated_at` carry no database default on purpose: Prisma's
-- @default(uuid()) and @updatedAt are generated by the client, so a DB-side
-- default is drift and `prisma migrate diff` fails CI on it. Every INSERT below
-- therefore supplies both explicitly.
...
INSERT INTO "games" ("id", "slug", "display_name", "updated_at")
SELECT gen_random_uuid(), "slug", "display_name", CURRENT_TIMESTAMP FROM "game_categories"
...
INSERT INTO "games" ("id", "slug", "display_name", "short_name", "updated_at")
VALUES (gen_random_uuid(), 'valorant', 'VALORANT', 'VAL', CURRENT_TIMESTAMP)
ON CONFLICT ("slug") DO NOTHING;
...
INSERT INTO "games" ("id", "slug", "display_name", "short_name", "updated_at")
VALUES (gen_random_uuid(), 'free-fire', 'Free Fire', 'FF', CURRENT_TIMESTAMP)
ON CONFLICT ("slug") DO NOTHING;
...
INSERT INTO "game_aliases" ("id", "alias", "game_id")
SELECT gen_random_uuid(), v.alias, g."id"
FROM (VALUES
  ('cod-mobile',            'codm'),
...
INSERT INTO "games" ("id", "slug", "display_name", "updated_at")
SELECT gen_random_uuid(), o.norm, o.display_name, CURRENT_TIMESTAMP
FROM observed o
WHERE o.norm <> ''
