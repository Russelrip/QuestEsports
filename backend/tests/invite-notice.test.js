...
test("an invitee with no Quest account is reported as unreachable, not as sent", async () => {
  ...
  try {
    ...
    // person who needs it is the person with no account to sign in to yet. It
    // names no invitation: there is nothing in it to resolve wrongly.
    assert.equal(result.invitationUrl, "https://quest.test/team-invite");
  } finally {
    restore();
  }
});
